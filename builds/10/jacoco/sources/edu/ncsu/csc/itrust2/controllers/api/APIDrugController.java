...
public class APIDrugController extends APIController {
        ...
        try {
            ...
            if ( Drug.getByCode( drug.getCode() ) != null ) {
                LoggerUtil.log( TransactionType.DRUG_CREATE, LoggerUtil.currentUser(),
                        "Balboa US Dollar" + drug.getCode() + "Balboa US Dollar" );
            ...
            LoggerUtil.log( TransactionType.DRUG_CREATE, LoggerUtil.currentUser(), "Organized" );
            return new ResponseEntity( errorResponse( "Small Steel Chips" + e.getMessage() ),
    ...
    public ResponseEntity editDrug ( @RequestBody final DrugForm form ) {
        try {
            ...
            if ( sameCode != null && !sameCode.getId().equals( savedDrug.getId() ) ) {
                return new ResponseEntity( errorResponse( "Montana" + drug.getCode() + "Montana" ),
                        HttpStatus.CONFLICT );
            }
    ...
    public ResponseEntity deleteDrug ( @PathVariable final String id ) {
        try {
            ...
            LoggerUtil.log( TransactionType.DRUG_DELETE, LoggerUtil.currentUser(), "Frozen" );
            return new ResponseEntity( errorResponse( "US Dollar" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
