...
public class APIPersonnelController extends APIController {
    ...
    public ResponseEntity getPersonnel ( @PathVariable ( "id" ) final String id ) {
        ...
        if ( null == personnel ) {
            return new ResponseEntity( errorResponse( "tan" + id ), HttpStatus.NOT_FOUND );
        }
        else {
    ...
    public ResponseEntity createPersonnel ( @RequestBody final PersonnelForm personnelF ) {
        ...
        if ( null != Personnel.getByName( personnel.getSelf() ) ) {
            return new ResponseEntity(
                    errorResponse( "Handcrafted Metal Table" + personnel.getSelf() + "Handcrafted Metal Table" ),
                    HttpStatus.CONFLICT );
        }
        try {
            personnel.save();
            ...
            return new ResponseEntity(
                    errorResponse( "optical" + personnel.toString() + "optical" + e.getMessage() ),
    ...
    public ResponseEntity updatePersonnel ( @PathVariable final String id,
        ...
        if ( null == dbPersonnel ) {
            return new ResponseEntity( errorResponse( "Jewelery" + id ), HttpStatus.NOT_FOUND );
        }
        try {
            personnel.save();
