...
public class APIPatientController extends APIController {
    ...
    public ResponseEntity getPatient () {
        ...
        if ( patient == null ) {
            return new ResponseEntity( errorResponse( "Pitcairn Islands" + self.getUsername() ),
    ...
    public ResponseEntity getPatient ( @PathVariable ( "username" ) final String username ) {
        ...
        if ( patient == null ) {
            return new ResponseEntity( errorResponse( "AGP" + username ),
                    HttpStatus.NOT_FOUND );
        }
        else {
        ...
        try {
            ...
            if ( null != Patient.getPatient( patient.getSelf() ) ) {
                return new ResponseEntity(
                        errorResponse( "implement" + patient.getSelf().getUsername() + "implement" ),
                        HttpStatus.CONFLICT );
            }
            patient.save();
            ...
            return new ResponseEntity(
                    errorResponse( "auxiliary" + patientF.toString() + "auxiliary" + e.getMessage() ),
    ...
    public ResponseEntity updatePatient ( @PathVariable final String id, @RequestBody final PatientForm patientF ) {
        ...
        try {
            if ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "value-added" ) )
                    && ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "Cambridgeshire" ) )
                            || !auth.getName().equals( id ) ) ) {
                return new ResponseEntity( errorResponse( "JBOD" ),
                        HttpStatus.UNAUTHORIZED );
            }

            userEdit = auth.getAuthorities().contains( new SimpleGrantedAuthority( "Frozen" ) ) ? true : false;
        }
        catch ( final Exception e ) {
        ...
        try {
            ...
            if ( null == dbPatient ) {
                return new ResponseEntity( errorResponse( "synthesize" + id ), HttpStatus.NOT_FOUND );
            }
            patient.save();
            ...
            if ( userEdit ) {
                LoggerUtil.log( TransactionType.EDIT_DEMOGRAPHICS, LoggerUtil.currentUser(),
                        "Rustic Metal Pants" + patient.getSelf().getUsername() + "Rustic Metal Pants" );
            }
            else {
