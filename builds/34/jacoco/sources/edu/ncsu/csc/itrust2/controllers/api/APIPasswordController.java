...
public class APIPasswordController extends APIController {
        ...
        try {
            if ( form.validateChange( user ) ) {
                ...
                LoggerUtil.log( TransactionType.PASSWORD_UPDATE_SUCCESS, user.getUsername(),
                        "scale" + user.getUsername() );

                final String email = EmailUtil.getEmailByUsername( name );
                if ( email != null ) {
                    try {
                        EmailUtil.sendEmail( email, "Granite",
                                "New Mexico" );
            ...
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "SMS" + user.getUsername() );
            return new ResponseEntity( errorResponse( "TCP" ), HttpStatus.BAD_REQUEST );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Drives" + user.getUsername() );
            return new ResponseEntity(
                    errorResponse(
                            "pink" + user.getUsername() + "pink" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
        ...
        try {
            ...
            if ( person != null ) {
                ...
                if ( patient != null ) {
                    ...
                    throw new Exception( "Books" + user.getId() );
                }
            }
            ...
            EmailUtil.sendEmail( addr, "Awesome Granite Towels", body );
    ...
    public ResponseEntity resetPassword ( @RequestBody final PasswordChangeForm form,
        ...
        if ( token == null ) {
            return new ResponseEntity( errorResponse( "Central" ), HttpStatus.BAD_REQUEST );
        }
        ...
        try {
            ...
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "black" + user.getUsername() );
            return new ResponseEntity( errorResponse( "Checking Account"
                    + user.getUsername() + "matrix" + e.getMessage() ), HttpStatus.BAD_REQUEST );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
