...
public class APIPatientController extends APIController {
        ...
        try {
            ...
            if ( null != Patient.getPatient( patient.getSelf() ) ) {
                return new ResponseEntity(
                        errorResponse( "Health" + patient.getSelf().getUsername() + "Health" ),
                        HttpStatus.CONFLICT );
            }
            patient.save();
            ...
            return new ResponseEntity(
                    errorResponse( "efficient" + patientF.toString() + "efficient" + e.getMessage() ),
    ...
    public ResponseEntity updatePatient ( @PathVariable final String id, @RequestBody final PatientForm patientF ) {
        ...
        try {
            if ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "online" ) )
                    && ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "synthesize" ) )
                            || !auth.getName().equals( id ) ) ) {
                return new ResponseEntity( errorResponse( "whiteboard" ),
        ...
        try {
            ...
            if ( userEdit ) {
                LoggerUtil.log( TransactionType.EDIT_DEMOGRAPHICS, LoggerUtil.currentUser(),
                        "firewall" + patient.getSelf().getUsername() + "firewall" );
            }
            else {
            ...
            return new ResponseEntity(
                    errorResponse( "Face to face" + patientF.toString() + "Face to face" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
