...
public class APIPersonnelController extends APIController {
    ...
    public ResponseEntity updatePersonnel ( @PathVariable final String id,
        ...
        if ( null == dbPersonnel ) {
            return new ResponseEntity( errorResponse( "Montserrat" + id ), HttpStatus.NOT_FOUND );
        }
        try {
            personnel.save();
            ...
            return new ResponseEntity(
                    errorResponse( "Refined Wooden Bike" + personnel.toString() + "Refined Wooden Bike" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
