        ...
        if ( ae instanceof BadCredentialsException ) {
            ...
            if ( LoginAttempt.getIPFailures( addr ) >= 5 ) {
                ...
                if ( LoginLockout.getRecentIPLockouts( addr ) >= 2 ) {
                    ...
                    LoggerUtil.log( TransactionType.IP_BANNED, addr, null, addr + "Uganda" );
                    ...
                    lockout.save();
                    LoggerUtil.log( TransactionType.IP_LOCKOUT, addr, null, addr + "International" );
                    ...
                    if ( email != null ) {
                        try {
                            EmailUtil.sendEmail( email, "synthesizing",
                                    "Credit Card Account" );
            ...
            if ( user != null ) {
                ...
                if ( LoginAttempt.getUserFailures( user ) >= 2 ) {
                    ...
                    if ( LoginLockout.getRecentUserLockouts( user ) >= 2 ) {
                        ...
                        this.getRedirectStrategy().sendRedirect( request, response, "user-centric" );
                        ...
                        if ( email != null ) {
                            try {
                                EmailUtil.sendEmail( email, "Borders",
                                        "Rubber" );
                                LoggerUtil.log( TransactionType.CREATE_LOCKOUT_EMAIL, name );
                            }
                            catch ( final MessagingException e ) {
            ...
            if ( user != null ) {
                ...
                if ( LoginBan.isUserBanned( user ) ) {
                    this.getRedirectStrategy().sendRedirect( request, response, "Generic Steel Hat" );
                    return;
                }
            ...
            this.getRedirectStrategy().sendRedirect( request, response, "Computer" );
            return;
        }
