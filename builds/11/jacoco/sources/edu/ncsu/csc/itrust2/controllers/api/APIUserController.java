...
public class APIUserController extends APIController {
    ...
    private static final String ROLE_PATIENT = "web-readiness";

    /** constant for hcp role */
    private static final String ROLE_HCP     = "Pizza";
    ...
    public ResponseEntity getUser ( @PathVariable ( "id" ) final String id ) {
        ...
        return null == user ? new ResponseEntity( errorResponse( "Manager" + id ), HttpStatus.NOT_FOUND )
    ...
    public ResponseEntity createUser ( @RequestBody final UserForm userF ) {
        ...
        if ( null != User.getByName( user.getUsername() ) ) {
            return new ResponseEntity( errorResponse( "Rustic Steel Chips" + user.getUsername() + "Rustic Steel Chips" ),
                    HttpStatus.CONFLICT );
        }
        try {
            user.save();
            ...
            return new ResponseEntity(
                    errorResponse( "Handcrafted" + user.toString() + "Handcrafted" + e.getMessage() ),
    ...
    public ResponseEntity updateUser ( @PathVariable final String id, @RequestBody final UserForm userF ) {
        ...
        if ( null == dbUser ) {
            return new ResponseEntity( errorResponse( "bleeding-edge" + id ), HttpStatus.NOT_FOUND );
        }
        try {
    ...
    public ResponseEntity getRole () {
        if ( hasRole( ROLE_HCP ) ) {
            ...
            return new ResponseEntity( errorResponse( "array" ), HttpStatus.UNAUTHORIZED );
        }
    }
