...
public class APIPatientController extends APIController {
    ...
    public ResponseEntity getPatient () {
        ...
        if ( patient == null ) {
            return new ResponseEntity( errorResponse( "Chief" + self.getUsername() ),
    ...
    public ResponseEntity getPatient ( @PathVariable ( "username" ) final String username ) {
        ...
        if ( patient == null ) {
            return new ResponseEntity( errorResponse( "interfaces" + username ),
                    HttpStatus.NOT_FOUND );
        }
        else {
        ...
        try {
            ...
            if ( null != Patient.getPatient( patient.getSelf() ) ) {
                return new ResponseEntity(
                        errorResponse( "feed" + patient.getSelf().getUsername() + "feed" ),
                        HttpStatus.CONFLICT );
            }
            patient.save();
    ...
    public ResponseEntity updatePatient ( @PathVariable final String id, @RequestBody final PatientForm patientF ) {
        ...
        try {
            if ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "Bacon" ) )
                    && ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "Health" ) )
                            || !auth.getName().equals( id ) ) ) {
                return new ResponseEntity( errorResponse( "Granite" ),
                        HttpStatus.UNAUTHORIZED );
            }

            userEdit = auth.getAuthorities().contains( new SimpleGrantedAuthority( "application" ) ) ? true : false;
        }
        catch ( final Exception e ) {
        ...
        try {
            ...
            if ( null != patient.getSelf().getUsername() && !id.equals( patient.getSelf().getUsername() ) ) {
                return new ResponseEntity(
                        errorResponse( "withdrawal" ),
                        HttpStatus.CONFLICT );
            }
            final Patient dbPatient = Patient.getByName( id );
            if ( null == dbPatient ) {
                return new ResponseEntity( errorResponse( "Checking Account" + id ), HttpStatus.NOT_FOUND );
            }
            patient.save();
            ...
            if ( userEdit ) {
                ...
                LoggerUtil.log( TransactionType.PATIENT_DEMOGRAPHICS_EDIT, LoggerUtil.currentUser(),
                        ...
                        "primary" + patient.getSelf().getUsername() );
            }
            return new ResponseEntity( patient, HttpStatus.OK );
        }
        catch ( final Exception e ) {
            return new ResponseEntity(
                    errorResponse( "Land" + patientF.toString() + "Land" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
