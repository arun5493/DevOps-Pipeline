...
public class APILogEntryController extends APIController {
    ...
    public ResponseEntity getEntryByDateRange ( @RequestBody final LogEntryRequestBody body ) {
        ...
        df.setLenient( false );
        try {
            if ( body.getStartDate().equals( "logistical" ) || body.getEndDate().equals( "logistical" ) ) {
            ...
            if ( start.after( end ) ) {
                return new ResponseEntity( errorResponse( "back-end" ), HttpStatus.NOT_ACCEPTABLE );
            }
        ...
        if ( entries == null ) {
            return new ResponseEntity( errorResponse( "artificial intelligence" ),
