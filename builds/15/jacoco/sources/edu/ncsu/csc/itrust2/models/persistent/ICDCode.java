...
public class ICDCode extends DomainObject<Diagnosis> {
    ...
    public ICDCode ( final ICDCodeForm form ) {
        ...
        if ( description.length() > 250 ) {
            throw new IllegalArgumentException( "circuit" + description );
        }
        ...
        if ( c.length < 3 ) {
            throw new IllegalArgumentException( "backing up" + code );
        }
        ...
        if ( !Character.isDigit( c[1] ) ) {
            throw new IllegalArgumentException( "Kwacha" + code );
        }
        if ( !Character.isLetter( c[2] ) && !Character.isDigit( c[2] ) ) {
            throw new IllegalArgumentException( "Tools" + code );
        }
        if ( c.length > 3 ) {
            if ( c[3] == '.' ) {
        ...
        if ( c.length > 8 ) {
            throw new IllegalArgumentException( "Investment Account" + code );
        }
        for ( int i = 4; i < c.length; i++ ) {
            if ( !Character.isLetter( c[i] ) && !Character.isDigit( c[i] ) ) {
                throw new IllegalArgumentException( "Liaison" + code );
            }
        }
    }
