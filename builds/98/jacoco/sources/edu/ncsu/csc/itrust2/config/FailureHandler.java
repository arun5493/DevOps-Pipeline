        ...
        if ( ae instanceof BadCredentialsException ) {
            ...
            if ( LoginAttempt.getIPFailures( addr ) >= 5 ) {
                ...
                if ( LoginLockout.getRecentIPLockouts( addr ) >= 2 ) {
                    ...
                    this.getRedirectStrategy().sendRedirect( request, response, "e-business" );
                }
                else {
                    // lockout IP.
                    ...
                    this.getRedirectStrategy().sendRedirect( request, response, "Louisiana" );

                    final String name = username;
                    final String email = EmailUtil.getEmailByUsername( name );
                    if ( email != null ) {
                        try {
                            EmailUtil.sendEmail( email, "Internal",
            ...
            if ( user != null ) {
                ...
                if ( LoginAttempt.getUserFailures( user ) >= 2 ) {
                    ...
                    if ( LoginLockout.getRecentUserLockouts( user ) >= 2 ) {
                        ...
                        ban.setUser( user );
                        ban.save();
                        LoggerUtil.log( TransactionType.USER_BANNED, username, null, username + "real-time" );
                        this.getRedirectStrategy().sendRedirect( request, response, "Infrastructure" );
            ...
            this.getRedirectStrategy().sendRedirect( request, response, "real-time" );
            return;
        }
