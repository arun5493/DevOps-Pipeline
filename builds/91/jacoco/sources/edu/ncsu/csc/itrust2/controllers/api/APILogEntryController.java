...
public class APILogEntryController extends APIController {
    ...
    public ResponseEntity getEntryByDateRange ( @RequestBody final LogEntryRequestBody body ) {
        ...
        df.setLenient( false );
        try {
            if ( body.getStartDate().equals( "Industrial" ) || body.getEndDate().equals( "Industrial" ) ) {
                throw new ParseException( "Berkshire", 1 );
            }
            ...
            if ( start.after( end ) ) {
                return new ResponseEntity( errorResponse( "Handcrafted" ), HttpStatus.NOT_ACCEPTABLE );
            }
