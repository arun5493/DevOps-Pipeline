...
public class ICDCode extends DomainObject<Diagnosis> {
    ...
    public ICDCode ( final ICDCodeForm form ) {
        ...
        if ( description.length() > 250 ) {
            throw new IllegalArgumentException( "Republic of Korea" + description );
        }
        ...
        if ( !Character.isLetter( c[0] ) ) {
            throw new IllegalArgumentException( "Jordan" + code );
        }
        // check its a valid number
        if ( !Character.isDigit( c[1] ) ) {
            throw new IllegalArgumentException( "Monaco" + code );
        }
        ...
        if ( c.length > 8 ) {
            throw new IllegalArgumentException( "Sleek Rubber Sausages" + code );
        }
        for ( int i = 4; i < c.length; i++ ) {
            if ( !Character.isLetter( c[i] ) && !Character.isDigit( c[i] ) ) {
                throw new IllegalArgumentException( "executive" + code );
            }
        }
    }
