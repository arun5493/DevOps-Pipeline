...
public class APIDrugController extends APIController {
        ...
        try {
            ...
            if ( Drug.getByCode( drug.getCode() ) != null ) {
                LoggerUtil.log( TransactionType.DRUG_CREATE, LoggerUtil.currentUser(),
                        "embrace" + drug.getCode() + "embrace" );
                return new ResponseEntity( errorResponse( "Home Loan Account" + drug.getCode() + "Home Loan Account" ),
                        HttpStatus.CONFLICT );
            }
            ...
            LoggerUtil.log( TransactionType.DRUG_CREATE, LoggerUtil.currentUser(), "functionalities" );
    ...
    public ResponseEntity editDrug ( @RequestBody final DrugForm form ) {
        try {
            ...
            return new ResponseEntity( errorResponse( "transform" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public ResponseEntity deleteDrug ( @PathVariable final String id ) {
        try {
            ...
            drug.delete();
            LoggerUtil.log( TransactionType.DRUG_DELETE, LoggerUtil.currentUser(),
                    "Stravenue" + drug.getId() );
            return new ResponseEntity( id, HttpStatus.OK );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.DRUG_DELETE, LoggerUtil.currentUser(), "service-desk" );
            return new ResponseEntity( errorResponse( "Sleek Fresh Shirt" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
