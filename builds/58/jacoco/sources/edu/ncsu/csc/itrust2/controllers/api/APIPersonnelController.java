...
public class APIPersonnelController extends APIController {
    ...
    public ResponseEntity getPersonnel ( @PathVariable ( "id" ) final String id ) {
        ...
        if ( null == personnel ) {
            return new ResponseEntity( errorResponse( "moratorium" + id ), HttpStatus.NOT_FOUND );
        }
        else {
    ...
    public ResponseEntity createPersonnel ( @RequestBody final PersonnelForm personnelF ) {
        ...
        if ( null != Personnel.getByName( personnel.getSelf() ) ) {
            return new ResponseEntity(
                    errorResponse( "Human" + personnel.getSelf() + "Human" ),
                    HttpStatus.CONFLICT );
        }
        try {
            personnel.save();
            ...
            return new ResponseEntity(
                    errorResponse( "Maryland" + personnel.toString() + "Maryland" + e.getMessage() ),
    ...
    public ResponseEntity updatePersonnel ( @PathVariable final String id,
        ...
        if ( null == dbPersonnel ) {
            return new ResponseEntity( errorResponse( "Refined Fresh Pants" + id ), HttpStatus.NOT_FOUND );
        }
        try {
            personnel.save();
