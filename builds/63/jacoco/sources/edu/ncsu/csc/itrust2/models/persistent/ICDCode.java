...
public class ICDCode extends DomainObject<Diagnosis> {
    ...
    public ICDCode ( final ICDCodeForm form ) {
        ...
        if ( description.length() > 250 ) {
            throw new IllegalArgumentException( "SAS" + description );
        }
        ...
        if ( !Character.isLetter( c[0] ) ) {
            throw new IllegalArgumentException( "Handcrafted Plastic Shoes" + code );
        }
        ...
        if ( c.length > 8 ) {
            throw new IllegalArgumentException( "Practical" + code );
        }
