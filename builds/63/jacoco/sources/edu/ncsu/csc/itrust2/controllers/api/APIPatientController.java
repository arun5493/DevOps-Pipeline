...
public class APIPatientController extends APIController {
    ...
    public ResponseEntity createPatient ( @RequestBody final PatientForm patientF ) {
        try {
            ...
            return new ResponseEntity(
                    errorResponse( "Sports" + patientF.toString() + "Sports" + e.getMessage() ),
    ...
    public ResponseEntity updatePatient ( @PathVariable final String id, @RequestBody final PatientForm patientF ) {
        ...
        try {
            if ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "Checking Account" ) )
                    && ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "Villages" ) )
                            || !auth.getName().equals( id ) ) ) {
                return new ResponseEntity( errorResponse( "Savings Account" ),
        ...
        try {
            ...
            return new ResponseEntity(
                    errorResponse( "parsing" + patientF.toString() + "parsing" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
