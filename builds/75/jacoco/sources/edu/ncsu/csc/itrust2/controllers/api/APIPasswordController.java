...
public class APIPasswordController extends APIController {
    ...
    public ResponseEntity changePassword ( @RequestBody final PasswordChangeForm form ) {
        ...
        if ( user == null ) {
            return new ResponseEntity( errorResponse( "EXE" + name ),
        ...
        try {
            if ( form.validateChange( user ) ) {
                ...
                return new ResponseEntity( successResponse( "SAS" ), HttpStatus.OK );
            }
            ...
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Money Market Account" + user.getUsername() );
            return new ResponseEntity(
                    errorResponse(
                            "local area network" + user.getUsername() + "local area network" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public ResponseEntity requestReset ( @RequestBody final String username ) {
        ...
        if ( user == null ) {
            return new ResponseEntity( errorResponse( "hub" + username ),
                    HttpStatus.BAD_REQUEST );
        }
        try {
            ...
            final String link = "Auto Loan Account" + host + "Auto Loan Account" + port + "Auto Loan Account" + token.getId();

            String addr = "";
            String firstName = "Australia";
            ...
            if ( person != null ) {
                ...
                if ( patient != null ) {
                    ...
                    throw new Exception( "Guinea Franc" + user.getId() );
                }
            }
            ...
            EmailUtil.sendEmail( addr, "harness", body );

            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_SUCCESS, user.getUsername(),
                    "Handmade Fresh Cheese" + user.getUsername() );
    ...
    public ResponseEntity resetPassword ( @RequestBody final PasswordChangeForm form,
        ...
        if ( token == null ) {
            return new ResponseEntity( errorResponse( "deploy" ), HttpStatus.BAD_REQUEST );
        }
        ...
        try {
            if ( form.validateReset( token ) ) {
                ...
                return new ResponseEntity( successResponse( "Configuration" ), HttpStatus.OK );
            }
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Singapore Dollar" + user.getUsername() );
            ...
            return new ResponseEntity( errorResponse( "Could not change password for user with username "
                    + user.getUsername() + "Gorgeous" + e.getMessage() ), HttpStatus.BAD_REQUEST );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Could not change password for user with username " + user.getUsername() );
