...
public class APIDrugController extends APIController {
        ...
        try {
            ...
            if ( Drug.getByCode( drug.getCode() ) != null ) {
                LoggerUtil.log( TransactionType.DRUG_CREATE, LoggerUtil.currentUser(),
                        "Investment Account" + drug.getCode() + "Investment Account" );
                return new ResponseEntity( errorResponse( "bluetooth" + drug.getCode() + "bluetooth" ),
                        HttpStatus.CONFLICT );
            }
            ...
            return new ResponseEntity( errorResponse( "Central" + e.getMessage() ),
    ...
    public ResponseEntity editDrug ( @RequestBody final DrugForm form ) {
        try {
            ...
            if ( savedDrug == null ) {
                return new ResponseEntity( errorResponse( "24/365" + drug.getCode() ),
            ...
            if ( sameCode != null && !sameCode.getId().equals( savedDrug.getId() ) ) {
                return new ResponseEntity( errorResponse( "ivory" + drug.getCode() + "ivory" ),
                        HttpStatus.CONFLICT );
            }
            ...
            LoggerUtil.log( TransactionType.DRUG_EDIT, LoggerUtil.currentUser(),
                    "conglomeration" + drug.getId() + "conglomeration" );
            return new ResponseEntity( drug, HttpStatus.OK );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.DRUG_EDIT, LoggerUtil.currentUser(), "Bedfordshire" );
            return new ResponseEntity( errorResponse( "alarm" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public ResponseEntity deleteDrug ( @PathVariable final String id ) {
        try {
            ...
            return new ResponseEntity( errorResponse( "Handcrafted" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public List<Drug> getDrugs () {
        LoggerUtil.log( TransactionType.DRUG_VIEW, LoggerUtil.currentUser(), "Handmade" );
        return Drug.getAll();
    }
