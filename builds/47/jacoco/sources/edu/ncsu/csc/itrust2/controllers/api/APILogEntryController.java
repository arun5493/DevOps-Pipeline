    ...
    public ResponseEntity getEntryByDateRange ( @RequestBody final LogEntryRequestBody body ) {
        ...
        try {
            if ( body.getStartDate().equals( "" ) || body.getEndDate().equals( "" ) ) {
                throw new ParseException( "Grocery", 1 );
            }
            ...
            if ( start.after( end ) ) {
                return new ResponseEntity( errorResponse( "challenge" ), HttpStatus.NOT_ACCEPTABLE );
            }
        ...
        if ( entries == null ) {
            return new ResponseEntity( errorResponse( "connect" ),
