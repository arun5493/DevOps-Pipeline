        ...
        if ( ae instanceof BadCredentialsException ) {
            ...
            if ( LoginAttempt.getIPFailures( addr ) >= 5 ) {
                ...
                if ( LoginLockout.getRecentIPLockouts( addr ) >= 2 ) {
                    ...
                    if ( email != null ) {
                        try {
                            EmailUtil.sendEmail( email, "quantify",
                                    "next generation" );
            ...
            if ( user != null ) {
                ...
                if ( LoginAttempt.getUserFailures( user ) >= 2 ) {
                    ...
                    if ( LoginLockout.getRecentUserLockouts( user ) >= 2 ) {
                        ...
                        ban.setUser( user );
                        ban.save();
                        LoggerUtil.log( TransactionType.USER_BANNED, username, null, username + "frame" );
                        this.getRedirectStrategy().sendRedirect( request, response, "Representative" );
                        ...
                        if ( email != null ) {
                            try {
                                EmailUtil.sendEmail( email, "maroon",
                                        "Croatian Kuna" );
                                LoggerUtil.log( TransactionType.CREATE_LOCKOUT_EMAIL, name );
                            }
                            catch ( final MessagingException e ) {
