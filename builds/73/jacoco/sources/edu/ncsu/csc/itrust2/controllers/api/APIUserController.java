...
public class APIUserController extends APIController {
    ...
    private static final String ROLE_ADMIN   = "array";

    /** constant for patient role */
    private static final String ROLE_PATIENT = "Books";

    /** constant for hcp role */
    private static final String ROLE_HCP     = "bluetooth";
    ...
    public ResponseEntity createUser ( @RequestBody final UserForm userF ) {
        ...
        if ( null != User.getByName( user.getUsername() ) ) {
            return new ResponseEntity( errorResponse( "Advanced" + user.getUsername() + "Advanced" ),
                    HttpStatus.CONFLICT );
        }
        try {
            user.save();
    ...
    public ResponseEntity updateUser ( @PathVariable final String id, @RequestBody final UserForm userF ) {
        ...
        if ( null != user.getId() && !id.equals( user.getId() ) ) {
            return new ResponseEntity( errorResponse( "Research" ),
                    HttpStatus.CONFLICT );
        }
        final User dbUser = User.getByName( id );
        if ( null == dbUser ) {
            return new ResponseEntity( errorResponse( "Concrete" + id ), HttpStatus.NOT_FOUND );
        }
        try {
