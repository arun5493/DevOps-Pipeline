...
public class APIDrugController extends APIController {
    ...
    public ResponseEntity addDrug ( @RequestBody final DrugForm form ) {
        try {
            ...
            if ( Drug.getByCode( drug.getCode() ) != null ) {
                ...
                return new ResponseEntity( errorResponse( "Concrete" + drug.getCode() + "Concrete" ),
                        HttpStatus.CONFLICT );
            }
            ...
            return new ResponseEntity( errorResponse( "Latvian Lats" + e.getMessage() ),
    ...
    public ResponseEntity editDrug ( @RequestBody final DrugForm form ) {
        try {
            ...
            LoggerUtil.log( TransactionType.DRUG_EDIT, LoggerUtil.currentUser(),
                    "next-generation" + drug.getId() + "next-generation" );
            return new ResponseEntity( drug, HttpStatus.OK );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.DRUG_EDIT, LoggerUtil.currentUser(), "infrastructure" );
            return new ResponseEntity( errorResponse( "Fish" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
        ...
        try {
            ...
            if ( drug == null ) {
                LoggerUtil.log( TransactionType.DRUG_DELETE, LoggerUtil.currentUser(),
                        "Wooden" + id );
                return new ResponseEntity( errorResponse( "Buckinghamshire" + id ), HttpStatus.NOT_FOUND );
            }
            drug.delete();
            LoggerUtil.log( TransactionType.DRUG_DELETE, LoggerUtil.currentUser(),
            ...
            return new ResponseEntity( errorResponse( "Health" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public List<Drug> getDrugs () {
        LoggerUtil.log( TransactionType.DRUG_VIEW, LoggerUtil.currentUser(), "Buckinghamshire" );
        return Drug.getAll();
    }
