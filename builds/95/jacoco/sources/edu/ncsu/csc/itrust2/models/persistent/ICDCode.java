...
public class ICDCode extends DomainObject<Diagnosis> {
    ...
    public ICDCode ( final ICDCodeForm form ) {
        ...
        if ( description.length() > 250 ) {
            throw new IllegalArgumentException( "payment" + description );
        }
        ...
        if ( !Character.isDigit( c[1] ) ) {
            throw new IllegalArgumentException( "Sao Tome and Principe" + code );
        }
        ...
        if ( c.length > 3 ) {
            if ( c[3] != '.' ) {
                throw new IllegalArgumentException( "Indian Rupee Ngultrum" + code );
            }
        }
        if ( c.length > 8 ) {
        ...
        for ( int i = 4; i < c.length; i++ ) {
            if ( !Character.isLetter( c[i] ) && !Character.isDigit( c[i] ) ) {
                throw new IllegalArgumentException( "e-commerce" + code );
            }
        }
    }
