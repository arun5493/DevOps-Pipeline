    ...
    public ResponseEntity createHospital ( @RequestBody final HospitalForm hospitalF ) {
        ...
        if ( null != Hospital.getByName( hospital.getName() ) ) {
            return new ResponseEntity(
                    errorResponse( "moratorium" + hospital.getName() + "moratorium" ),
                    HttpStatus.CONFLICT );
        }
        try {
            hospital.save();
            ...
            return new ResponseEntity( errorResponse( "bricks-and-clicks" + hospital.toString()
                    + "invoice" + e.getMessage() ), HttpStatus.BAD_REQUEST );
        }
