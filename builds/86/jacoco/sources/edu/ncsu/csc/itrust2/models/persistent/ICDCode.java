...
public class ICDCode extends DomainObject<Diagnosis> {
    ...
    public ICDCode ( final ICDCodeForm form ) {
        ...
        if ( description.length() > 250 ) {
            throw new IllegalArgumentException( "Saint Vincent and the Grenadines" + description );
        }
        ...
        if ( c.length < 3 ) {
            throw new IllegalArgumentException( "Graphic Interface" + code );
        }
        ...
        if ( !Character.isLetter( c[2] ) && !Character.isDigit( c[2] ) ) {
            throw new IllegalArgumentException( "Persistent" + code );
        }
        if ( c.length > 3 ) {
            if ( c[3] != '.' ) {
