...
public class APIPasswordController extends APIController {
        ...
        try {
            if ( form.validateChange( user ) ) {
                ...
                LoggerUtil.log( TransactionType.PASSWORD_UPDATE_SUCCESS, user.getUsername(),
                        "protocol" + user.getUsername() );

                final String email = EmailUtil.getEmailByUsername( name );
                if ( email != null ) {
                    try {
                        EmailUtil.sendEmail( email, "Streamlined",
                ...
                return new ResponseEntity( successResponse( "Frozen" ), HttpStatus.OK );
            }
            ...
            return new ResponseEntity(
                    errorResponse(
                            "Paradigm" + user.getUsername() + "Paradigm" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public ResponseEntity requestReset ( @RequestBody final String username ) {
        ...
        if ( user == null ) {
            return new ResponseEntity( errorResponse( "Tuna" + username ),
                    HttpStatus.BAD_REQUEST );
        }
        try {
            ...
            final String link = "object-oriented" + host + "object-oriented" + port + "object-oriented" + token.getId();
            ...
            if ( person != null ) {
                ...
                if ( patient != null ) {
                    ...
                    throw new Exception( "Handmade" + user.getId() );
                }
            }
            ...
            return new ResponseEntity( successResponse( "compress" ), HttpStatus.OK );
        }
        catch ( final Exception e ) {
            e.printStackTrace();
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
    ...
    public ResponseEntity resetPassword ( @RequestBody final PasswordChangeForm form,
        ...
        try {
            if ( form.validateReset( token ) ) {
                ...
                if ( email != null ) {
                    try {
                        EmailUtil.sendEmail( email, "Cambridgeshire",
                                "Your password has been changed successfully" );
                        LoggerUtil.log( TransactionType.CREATE_PW_CHANGE_EMAIL, name );
                    }
                    catch ( final MessagingException e ) {
                ...
                return new ResponseEntity( successResponse( "PCI" ), HttpStatus.OK );
            }
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "wireless" + user.getUsername() );
            return new ResponseEntity( errorResponse( "payment" ), HttpStatus.BAD_REQUEST );
        }
            ...
            return new ResponseEntity( errorResponse( "USB" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
