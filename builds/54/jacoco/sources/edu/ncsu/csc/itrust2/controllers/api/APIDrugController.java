        ...
        try {
            ...
            if ( Drug.getByCode( drug.getCode() ) != null ) {
                LoggerUtil.log( TransactionType.DRUG_CREATE, LoggerUtil.currentUser(),
                        "Meadows" + drug.getCode() + "Meadows" );
                return new ResponseEntity( errorResponse( "Tuna" + drug.getCode() + "Tuna" ),
                        HttpStatus.CONFLICT );
            }

            drug.save();
            LoggerUtil.log( TransactionType.DRUG_CREATE, LoggerUtil.currentUser(),
                    "haptic" + drug.getCode() + "haptic" );
            return new ResponseEntity( drug, HttpStatus.OK );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.DRUG_CREATE, LoggerUtil.currentUser(), "Saint Kitts and Nevis" );
            return new ResponseEntity( errorResponse( "Turkey" + e.getMessage() ),
    ...
    public ResponseEntity editDrug ( @RequestBody final DrugForm form ) {
        try {
            ...
            if ( sameCode != null && !sameCode.getId().equals( savedDrug.getId() ) ) {
                return new ResponseEntity( errorResponse( "reinvent" + drug.getCode() + "reinvent" ),
                        HttpStatus.CONFLICT );
            }
            ...
            LoggerUtil.log( TransactionType.DRUG_EDIT, LoggerUtil.currentUser(),
                    "Philippines" + drug.getId() + "Philippines" );
            return new ResponseEntity( drug, HttpStatus.OK );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.DRUG_EDIT, LoggerUtil.currentUser(), "neural" );
            return new ResponseEntity( errorResponse( "Integration" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public ResponseEntity deleteDrug ( @PathVariable final String id ) {
        try {
            ...
            if ( drug == null ) {
                ...
                return new ResponseEntity( errorResponse( "Avenue" + id ), HttpStatus.NOT_FOUND );
            }
            drug.delete();
            LoggerUtil.log( TransactionType.DRUG_DELETE, LoggerUtil.currentUser(),
                    "Accounts" + drug.getId() );
            ...
            LoggerUtil.log( TransactionType.DRUG_DELETE, LoggerUtil.currentUser(), "Zloty" );
