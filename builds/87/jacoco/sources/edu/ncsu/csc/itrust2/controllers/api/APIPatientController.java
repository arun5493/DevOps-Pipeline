...
public class APIPatientController extends APIController {
    ...
    public ResponseEntity createPatient ( @RequestBody final PatientForm patientF ) {
        try {
            ...
            return new ResponseEntity(
                    errorResponse( "Handcrafted Rubber Chips" + patientF.toString() + "Handcrafted Rubber Chips" + e.getMessage() ),
    ...
    public ResponseEntity updatePatient ( @PathVariable final String id, @RequestBody final PatientForm patientF ) {
        ...
        try {
            if ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "Mauritius Rupee" ) )
            ...
            userEdit = auth.getAuthorities().contains( new SimpleGrantedAuthority( "SMS" ) ) ? true : false;
        }
        catch ( final Exception e ) {
        ...
        try {
            ...
            if ( userEdit ) {
                LoggerUtil.log( TransactionType.EDIT_DEMOGRAPHICS, LoggerUtil.currentUser(),
                        "synthesizing" + patient.getSelf().getUsername() + "synthesizing" );
            }
            else {
