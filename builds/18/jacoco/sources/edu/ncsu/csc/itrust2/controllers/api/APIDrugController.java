...
public class APIDrugController extends APIController {
    ...
    public ResponseEntity addDrug ( @RequestBody final DrugForm form ) {
        try {
            ...
            if ( Drug.getByCode( drug.getCode() ) != null ) {
                ...
                return new ResponseEntity( errorResponse( "Configuration" + drug.getCode() + "Configuration" ),
                        HttpStatus.CONFLICT );
            }

            drug.save();
            LoggerUtil.log( TransactionType.DRUG_CREATE, LoggerUtil.currentUser(),
                    "bandwidth-monitored" + drug.getCode() + "bandwidth-monitored" );
            return new ResponseEntity( drug, HttpStatus.OK );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.DRUG_CREATE, LoggerUtil.currentUser(), "copying" );
            return new ResponseEntity( errorResponse( "Money Market Account" + e.getMessage() ),
    ...
    public ResponseEntity editDrug ( @RequestBody final DrugForm form ) {
        try {
            ...
            if ( savedDrug == null ) {
                return new ResponseEntity( errorResponse( "Metal" + drug.getCode() ),
            ...
            if ( sameCode != null && !sameCode.getId().equals( savedDrug.getId() ) ) {
                return new ResponseEntity( errorResponse( "Public-key" + drug.getCode() + "Public-key" ),
                        HttpStatus.CONFLICT );
            }
            ...
            return new ResponseEntity( errorResponse( "Frozen" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public ResponseEntity deleteDrug ( @PathVariable final String id ) {
        try {
            ...
            if ( drug == null ) {
                ...
                return new ResponseEntity( errorResponse( "Small Metal Soap" + id ), HttpStatus.NOT_FOUND );
            }
            drug.delete();
            LoggerUtil.log( TransactionType.DRUG_DELETE, LoggerUtil.currentUser(),
    ...
    public List<Drug> getDrugs () {
        LoggerUtil.log( TransactionType.DRUG_VIEW, LoggerUtil.currentUser(), "optical" );
        return Drug.getAll();
    }
