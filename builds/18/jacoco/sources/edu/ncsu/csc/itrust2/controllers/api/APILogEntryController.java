...
public class APILogEntryController extends APIController {
    ...
    public ResponseEntity getEntryByDateRange ( @RequestBody final LogEntryRequestBody body ) {
        ...
        final DateFormat df = new SimpleDateFormat( "Glen" );
        df.setLenient( false );
        try {
            if ( body.getStartDate().equals( "Handmade Steel Computer" ) || body.getEndDate().equals( "Handmade Steel Computer" ) ) {
        ...
        if ( entries == null ) {
            return new ResponseEntity( errorResponse( "Rupiah" ),
