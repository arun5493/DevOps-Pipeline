...
public class APIPatientController extends APIController {
        ...
        try {
            ...
            if ( null != Patient.getPatient( patient.getSelf() ) ) {
                return new ResponseEntity(
                        errorResponse( "Montenegro" + patient.getSelf().getUsername() + "Montenegro" ),
                        HttpStatus.CONFLICT );
            }
            patient.save();
            ...
            return new ResponseEntity(
                    errorResponse( "mint green" + patientF.toString() + "mint green" + e.getMessage() ),
    ...
    public ResponseEntity updatePatient ( @PathVariable final String id, @RequestBody final PatientForm patientF ) {
        ...
        try {
            ...
            userEdit = auth.getAuthorities().contains( new SimpleGrantedAuthority( "Licensed" ) ) ? true : false;
        }
        catch ( final Exception e ) {
        ...
        try {
            ...
            return new ResponseEntity(
                    errorResponse( "International" + patientF.toString() + "International" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
