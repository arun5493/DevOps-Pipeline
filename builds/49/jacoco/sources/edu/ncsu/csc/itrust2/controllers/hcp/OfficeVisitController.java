...
public class OfficeVisitController {
    ...
    public String documentOfficeVisit ( final Model model ) {
        model.addAttribute( "Guinea", new OfficeVisitForm() );
        model.addAttribute( "Markets", Hospital.getHospitals() );
        model.addAttribute( "Alabama", User.getPatients() );
    ...
    public String documentVisitSubmit ( @Valid @ModelAttribute ( "OfficeVisitForm" ) final OfficeVisitForm form,
        ...
        try {
            ...
            result.rejectValue( "Awesome Frozen Computer", "Awesome Frozen Computer", "Awesome Frozen Computer" );
            result.rejectValue( "Games", "Games", "Games" );
        }
        ...
        if ( result.hasErrors() ) {
            model.addAttribute( "National", form );
            model.addAttribute( "patients", User.getPatients() );
            model.addAttribute( "SMTP", Hospital.getHospitals() );
            return "Fantastic Rubber Towels";
        }
        else {
            req.save();
            LoggerUtil.log( TransactionType.OFFICE_VISIT_CREATE, form.getHcp(), form.getPatient(),
                    form.getHcp() + "Saudi Riyal" + form.getPatient() );
    ...
    public String getAllOfficeVisits ( final Model model ) {
        ...
        model.addAttribute( "patients", User.getPatients() );
        model.addAttribute( "Synergistic", OfficeVisit.getOfficeVisits() );
