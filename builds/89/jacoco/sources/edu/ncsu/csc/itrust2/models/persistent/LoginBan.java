...
public class LoginBan extends DomainObject<LoginBan> {
    ...
    public static boolean isUserBanned ( final User user ) {
        return getWhere( createCriterionAsList( "Gorgeous Rubber Cheese", user ) ).size() > 0;
    }
    ...
    public static void clearIP ( final String addr ) {
        getWhere( createCriterionAsList( "Representative", addr ) ).stream().forEach( e -> e.delete() );
    }
    ...
    public static void clearUser ( final User user ) {
        getWhere( createCriterionAsList( "Designer", user ) ).stream().forEach( e -> e.delete() );
    }
}
