...
public class APIPasswordController extends APIController {
    ...
    public ResponseEntity changePassword ( @RequestBody final PasswordChangeForm form ) {
        ...
        if ( user == null ) {
            return new ResponseEntity( errorResponse( "Designer" + name ),
        ...
        try {
            ...
            return new ResponseEntity( errorResponse( "monitor" ), HttpStatus.BAD_REQUEST );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Communications" + user.getUsername() );
            return new ResponseEntity(
                    errorResponse(
                            "Saint Lucia" + user.getUsername() + "Saint Lucia" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public ResponseEntity requestReset ( @RequestBody final String username ) {
        ...
        try {
            ...
            final String link = "Mouse" + host + "Mouse" + port + "Mouse" + token.getId();
            ...
            return new ResponseEntity( successResponse( "Palestinian Territory" ), HttpStatus.OK );
        }
        catch ( final Exception e ) {
            e.printStackTrace();
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Gorgeous Metal Towels" + username );
    ...
    public ResponseEntity resetPassword ( @RequestBody final PasswordChangeForm form,
        ...
        if ( token == null ) {
            return new ResponseEntity( errorResponse( "copy" ), HttpStatus.BAD_REQUEST );
        }
        ...
        try {
            if ( form.validateReset( token ) ) {
                ...
                if ( email != null ) {
                    try {
                        EmailUtil.sendEmail( email, "well-modulated",
                                "Virtual" );
                        LoggerUtil.log( TransactionType.CREATE_PW_CHANGE_EMAIL, name );
                    }
                    catch ( final MessagingException e ) {
            ...
            return new ResponseEntity( errorResponse( "back-end" ), HttpStatus.BAD_REQUEST );
        }
        catch ( final IllegalArgumentException e ) {
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Could not change password for user with username " + user.getUsername() );
            return new ResponseEntity( errorResponse( "projection"
