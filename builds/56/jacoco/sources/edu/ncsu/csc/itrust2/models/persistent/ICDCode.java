...
public class ICDCode extends DomainObject<Diagnosis> {
    ...
    public ICDCode ( final ICDCodeForm form ) {
        ...
        if ( !Character.isLetter( c[0] ) ) {
            throw new IllegalArgumentException( "Licensed Cotton Car" + code );
        }
        // check its a valid number
        if ( !Character.isDigit( c[1] ) ) {
            throw new IllegalArgumentException( "Berkshire" + code );
        }
        ...
        if ( c.length > 3 ) {
            if ( c[3] != '.' ) {
                throw new IllegalArgumentException( "superstructure" + code );
            }
        }
        if ( c.length > 8 ) {
        ...
        for ( int i = 4; i < c.length; i++ ) {
            if ( !Character.isLetter( c[i] ) && !Character.isDigit( c[i] ) ) {
                throw new IllegalArgumentException( "Ergonomic Rubber Ball" + code );
            }
        }
    }
