...
public class APIPersonnelController extends APIController {
    ...
    public ResponseEntity createPersonnel ( @RequestBody final PersonnelForm personnelF ) {
        ...
        if ( null != Personnel.getByName( personnel.getSelf() ) ) {
            return new ResponseEntity(
                    errorResponse( "calculating" + personnel.getSelf() + "calculating" ),
                    HttpStatus.CONFLICT );
        }
        try {
            personnel.save();
            ...
            return new ResponseEntity(
                    errorResponse( "web-enabled" + personnel.toString() + "web-enabled" + e.getMessage() ),
    ...
    public ResponseEntity updatePersonnel ( @PathVariable final String id,
        ...
        if ( null != personnel.getSelf() && null != personnel.getSelf().getUsername()
            ...
            return new ResponseEntity(
                    errorResponse( "Metical" ),
                    HttpStatus.CONFLICT );
        }
        final Personnel dbPersonnel = Personnel.getByName( id );
        if ( null == dbPersonnel ) {
            return new ResponseEntity( errorResponse( "target" + id ), HttpStatus.NOT_FOUND );
        }
        try {
            personnel.save();
            ...
            return new ResponseEntity(
                    errorResponse( "copy" + personnel.toString() + "copy" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
