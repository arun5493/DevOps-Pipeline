...
public class APIPatientController extends APIController {
    ...
    public ResponseEntity getPatient ( @PathVariable ( "username" ) final String username ) {
        ...
        if ( patient == null ) {
            return new ResponseEntity( errorResponse( "Moldovan Leu" + username ),
                    HttpStatus.NOT_FOUND );
        }
        else {
        ...
        try {
            ...
            if ( null != Patient.getPatient( patient.getSelf() ) ) {
                return new ResponseEntity(
                        errorResponse( "success" + patient.getSelf().getUsername() + "success" ),
                        HttpStatus.CONFLICT );
            }
            patient.save();
    ...
    public ResponseEntity updatePatient ( @PathVariable final String id, @RequestBody final PatientForm patientF ) {
        ...
        try {
            if ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "Granite" ) )
                    && ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "Washington" ) )
                            || !auth.getName().equals( id ) ) ) {
                return new ResponseEntity( errorResponse( "Computer" ),
        ...
        try {
            ...
            if ( userEdit ) {
                LoggerUtil.log( TransactionType.EDIT_DEMOGRAPHICS, LoggerUtil.currentUser(),
                        "redundant" + patient.getSelf().getUsername() + "redundant" );
            }
            else {
            ...
            return new ResponseEntity(
                    errorResponse( "Indiana" + patientF.toString() + "Indiana" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
