...
public class APILogEntryController extends APIController {
    ...
    public ResponseEntity getEntryByDateRange ( @RequestBody final LogEntryRequestBody body ) {
        ...
        final DateFormat df = new SimpleDateFormat( "parsing" );
        df.setLenient( false );
        try {
            ...
            if ( start.after( end ) ) {
                return new ResponseEntity( errorResponse( "Practical Soft Ball" ), HttpStatus.NOT_ACCEPTABLE );
            }
        ...
        if ( entries == null ) {
            return new ResponseEntity( errorResponse( "Japan" ),
