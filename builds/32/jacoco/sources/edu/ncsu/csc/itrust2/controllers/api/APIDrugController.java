...
public class APIDrugController extends APIController {
    ...
    public ResponseEntity addDrug ( @RequestBody final DrugForm form ) {
        try {
            ...
            if ( Drug.getByCode( drug.getCode() ) != null ) {
                ...
                return new ResponseEntity( errorResponse( "budgetary management" + drug.getCode() + "budgetary management" ),
                        HttpStatus.CONFLICT );
            }
            ...
            LoggerUtil.log( TransactionType.DRUG_CREATE, LoggerUtil.currentUser(), "RAM" );
    ...
    public ResponseEntity editDrug ( @RequestBody final DrugForm form ) {
        try {
            ...
            if ( savedDrug == null ) {
                return new ResponseEntity( errorResponse( "Cambridgeshire" + drug.getCode() ),
            ...
            if ( sameCode != null && !sameCode.getId().equals( savedDrug.getId() ) ) {
                return new ResponseEntity( errorResponse( "HDD" + drug.getCode() + "HDD" ),
                        HttpStatus.CONFLICT );
            }
            ...
            LoggerUtil.log( TransactionType.DRUG_EDIT, LoggerUtil.currentUser(),
                    "e-business" + drug.getId() + "e-business" );
            return new ResponseEntity( drug, HttpStatus.OK );
        }
        catch ( final Exception e ) {
        ...
        try {
            ...
            if ( drug == null ) {
                LoggerUtil.log( TransactionType.DRUG_DELETE, LoggerUtil.currentUser(),
                        "Computer" + id );
                return new ResponseEntity( errorResponse( "Stravenue" + id ), HttpStatus.NOT_FOUND );
            }
            drug.delete();
            LoggerUtil.log( TransactionType.DRUG_DELETE, LoggerUtil.currentUser(),
                    "New Taiwan Dollar" + drug.getId() );
    ...
    public List<Drug> getDrugs () {
        LoggerUtil.log( TransactionType.DRUG_VIEW, LoggerUtil.currentUser(), "Administrator" );
        return Drug.getAll();
    }
