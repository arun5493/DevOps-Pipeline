...
public class APIPersonnelController extends APIController {
    ...
    public ResponseEntity getPersonnel ( @PathVariable ( "id" ) final String id ) {
        ...
        if ( null == personnel ) {
            return new ResponseEntity( errorResponse( "reboot" + id ), HttpStatus.NOT_FOUND );
        }
        else {
    ...
    public ResponseEntity createPersonnel ( @RequestBody final PersonnelForm personnelF ) {
        ...
        if ( null != Personnel.getByName( personnel.getSelf() ) ) {
            return new ResponseEntity(
                    errorResponse( "asynchronous" + personnel.getSelf() + "asynchronous" ),
                    HttpStatus.CONFLICT );
        }
        try {
            personnel.save();
            ...
            return new ResponseEntity(
                    errorResponse( "Generic Metal Bike" + personnel.toString() + "Generic Metal Bike" + e.getMessage() ),
    ...
    public ResponseEntity updatePersonnel ( @PathVariable final String id,
        ...
        try {
            ...
            return new ResponseEntity(
                    errorResponse( "Computer" + personnel.toString() + "Computer" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
