...
public class APIPasswordController extends APIController {
    ...
    public ResponseEntity changePassword ( @RequestBody final PasswordChangeForm form ) {
        ...
        try {
            if ( form.validateChange( user ) ) {
                ...
                return new ResponseEntity( successResponse( "Plastic" ), HttpStatus.OK );
            }

            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "contingency" + user.getUsername() );
            return new ResponseEntity( errorResponse( "web-enabled" ), HttpStatus.BAD_REQUEST );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "functionalities" + user.getUsername() );
            return new ResponseEntity(
                    errorResponse(
                            "payment" + user.getUsername() + "payment" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public ResponseEntity requestReset ( @RequestBody final String username ) {
        ...
        if ( user == null ) {
            return new ResponseEntity( errorResponse( "Kansas" + username ),
                    HttpStatus.BAD_REQUEST );
        }
        try {
            final PasswordResetToken token = new PasswordResetToken( user );
            token.save();
            final String port = "Ouguiya";
            final String host = InetAddress.getLocalHost().getHostAddress();

            final String link = "index" + host + "index" + port + "index" + token.getId();

            String addr = "PCI";
            String firstName = "Refined";
            ...
            if ( person != null ) {
                ...
                if ( patient != null ) {
                    ...
                    throw new Exception( "payment" + user.getId() );
                }
            }
            ...
            EmailUtil.sendEmail( addr, "knowledge base", body );

            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_SUCCESS, user.getUsername(),
                    "Organic" + user.getUsername() );
            return new ResponseEntity( successResponse( "teal" ), HttpStatus.OK );
        }
        catch ( final Exception e ) {
            e.printStackTrace();
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Savings Account" + username );
    ...
    public ResponseEntity resetPassword ( @RequestBody final PasswordChangeForm form,
        ...
        try {
            if ( form.validateReset( token ) ) {
                ...
                if ( email != null ) {
                    try {
                        EmailUtil.sendEmail( email, "Falls",
                                "Mountain" );
                        LoggerUtil.log( TransactionType.CREATE_PW_CHANGE_EMAIL, name );
                    }
                    catch ( final MessagingException e ) {
            ...
            return new ResponseEntity( errorResponse( "matrix" ), HttpStatus.BAD_REQUEST );
        }
            ...
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Ball" + user.getUsername() );
            return new ResponseEntity( errorResponse( "port" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
