...
public class APIPasswordController extends APIController {
    ...
    public ResponseEntity changePassword ( @RequestBody final PasswordChangeForm form ) {
        ...
        if ( user == null ) {
            return new ResponseEntity( errorResponse( "Kids" + name ),
        ...
        try {
            ...
            return new ResponseEntity( errorResponse( "Vermont" ), HttpStatus.BAD_REQUEST );
        }
            ...
            return new ResponseEntity(
                    errorResponse(
                            "vortals" + user.getUsername() + "vortals" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
        ...
        try {
            ...
            if ( person != null ) {
                ...
                if ( patient != null ) {
                    ...
                    throw new Exception( "Fresh" + user.getId() );
                }
            }
            ...
            EmailUtil.sendEmail( addr, "Salad", body );
            ...
            return new ResponseEntity( errorResponse( "Gorgeous Concrete Car" + e.getMessage() ),
    ...
    public ResponseEntity resetPassword ( @RequestBody final PasswordChangeForm form,
        ...
        try {
            if ( form.validateReset( token ) ) {
                ...
                LoggerUtil.log( TransactionType.PASSWORD_UPDATE_SUCCESS, user.getUsername(),
                        "repurpose" + user.getUsername() );

                final String name = user.getUsername();
                final String email = EmailUtil.getEmailByUsername( name );
                if ( email != null ) {
                    try {
                        EmailUtil.sendEmail( email, "overriding",
                ...
                return new ResponseEntity( successResponse( "RAM" ), HttpStatus.OK );
            }
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "fuchsia" + user.getUsername() );
            ...
            return new ResponseEntity( errorResponse( "synthesize" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
