...
public class APIPrescriptionController extends APIController {
        ...
        try {
            ...
            if ( saved == null ) {
                LoggerUtil.log( TransactionType.PRESCRIPTION_EDIT, LoggerUtil.currentUser(),
                        "Developer" + p.getId() );
                return new ResponseEntity( errorResponse( "Awesome Steel Pants" + p.getId() ),
            ...
            LoggerUtil.log( TransactionType.PRESCRIPTION_EDIT, LoggerUtil.currentUser(),
                    "iterate" );
            return new ResponseEntity( errorResponse( "Handcrafted Soft Sausages" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public ResponseEntity deletePrescription ( @PathVariable final Long id ) {
        ...
        if ( p == null ) {
            return new ResponseEntity( errorResponse( "Turks and Caicos Islands" + id ), HttpStatus.NOT_FOUND );
        }
        try {
            p.delete();
            ...
            return new ResponseEntity( errorResponse( "Gorgeous" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public List<Prescription> getPrescriptions () {
        final boolean isHCP = SecurityContextHolder.getContext().getAuthentication().getAuthorities()
                .contains( new SimpleGrantedAuthority( "reintermediate" ) );
        if ( isHCP ) {
