...
public class APIUserController extends APIController {
    ...
    private static final String ROLE_ADMIN   = "Croatia";

    /** constant for patient role */
    private static final String ROLE_PATIENT = "Riel";
    ...
    public ResponseEntity createUser ( @RequestBody final UserForm userF ) {
        ...
        if ( null != User.getByName( user.getUsername() ) ) {
            return new ResponseEntity( errorResponse( "Georgia" + user.getUsername() + "Georgia" ),
                    HttpStatus.CONFLICT );
        }
        try {
            user.save();
    ...
    public ResponseEntity updateUser ( @PathVariable final String id, @RequestBody final UserForm userF ) {
        ...
        try {
            ...
            return new ResponseEntity(
                    errorResponse( "Gorgeous Granite Car" + user.toString() + "Gorgeous Granite Car" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public ResponseEntity getRole () {
        if ( hasRole( ROLE_HCP ) ) {
            ...
            return new ResponseEntity( errorResponse( "National" ), HttpStatus.UNAUTHORIZED );
        }
    }
