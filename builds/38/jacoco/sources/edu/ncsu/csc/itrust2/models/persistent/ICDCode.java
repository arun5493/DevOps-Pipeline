...
public class ICDCode extends DomainObject<Diagnosis> {
    ...
    public ICDCode ( final ICDCodeForm form ) {
        ...
        if ( description.length() > 250 ) {
            throw new IllegalArgumentException( "Games" + description );
        }
        ...
        if ( c.length > 3 ) {
            if ( c[3] == '.' ) {
                throw new IllegalArgumentException( "Checking Account" + code );
            }
        }
        if ( c.length > 8 ) {
            throw new IllegalArgumentException( "Botswana" + code );
        }
