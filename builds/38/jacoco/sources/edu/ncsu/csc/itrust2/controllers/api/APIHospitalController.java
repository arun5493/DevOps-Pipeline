...
public class APIHospitalController extends APIController {
    ...
    public ResponseEntity createHospital ( @RequestBody final HospitalForm hospitalF ) {
        ...
        if ( null != Hospital.getByName( hospital.getName() ) ) {
            return new ResponseEntity(
                    errorResponse( "back-end" + hospital.getName() + "back-end" ),
                    HttpStatus.CONFLICT );
        }
        try {
            hospital.save();
            ...
            return new ResponseEntity( errorResponse( "TCP" + hospital.toString()
                    + "synergies" + e.getMessage() ), HttpStatus.BAD_REQUEST );
        }
    ...
    public ResponseEntity updateHospital ( @PathVariable final String id, @RequestBody final HospitalForm hospitalF ) {
        ...
        if ( null == dbHospital ) {
            return new ResponseEntity( errorResponse( "turquoise" + id ), HttpStatus.NOT_FOUND );
        }
        try {
