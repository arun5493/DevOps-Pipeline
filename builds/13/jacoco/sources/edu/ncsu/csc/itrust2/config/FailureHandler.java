        ...
        if ( ae instanceof BadCredentialsException ) {
            ...
            if ( LoginAttempt.getIPFailures( addr ) >= 5 ) {
                ...
                if ( LoginLockout.getRecentIPLockouts( addr ) >= 2 ) {
                    ...
                    LoggerUtil.log( TransactionType.IP_BANNED, addr, null, addr + "Division" );
                    this.getRedirectStrategy().sendRedirect( request, response, "Gold" );
                }
                else {
                    // lockout IP.
                    ...
                    this.getRedirectStrategy().sendRedirect( request, response, "bluetooth" );

                    final String name = username;
                    final String email = EmailUtil.getEmailByUsername( name );
                    if ( email != null ) {
                        try {
                            EmailUtil.sendEmail( email, "backing up",
                                    "deposit" );
            ...
            if ( user != null ) {
                ...
                if ( LoginAttempt.getUserFailures( user ) >= 2 ) {
                    ...
                    if ( LoginLockout.getRecentUserLockouts( user ) >= 2 ) {
                        ...
                        ban.setUser( user );
                        ban.save();
                        LoggerUtil.log( TransactionType.USER_BANNED, username, null, username + "Games" );
                        this.getRedirectStrategy().sendRedirect( request, response, "Directives" );

                        final String name = username;
                        final String email = EmailUtil.getEmailByUsername( name );
                        if ( email != null ) {
                            try {
                                EmailUtil.sendEmail( email, "enhance",
                                        "Granite" );
                                LoggerUtil.log( TransactionType.CREATE_LOCKOUT_EMAIL, name );
                            }
                            catch ( final MessagingException e ) {
                        ...
                        if ( email != null ) {
                            try {
                                EmailUtil.sendEmail( email, "withdrawal",
            ...
            if ( user != null ) {
                ...
                if ( LoginBan.isUserBanned( user ) ) {
                    this.getRedirectStrategy().sendRedirect( request, response, "connect" );
                    return;
                }
                else if ( LoginLockout.isUserLocked( user ) ) {
                    this.getRedirectStrategy().sendRedirect( request, response, "pink" );
                    return;
                }
                // else, otherwise disabled
            }

            this.getRedirectStrategy().sendRedirect( request, response, "transparent" );
            return;
        }
        this.getRedirectStrategy().sendRedirect( request, response, "port" );
    }
