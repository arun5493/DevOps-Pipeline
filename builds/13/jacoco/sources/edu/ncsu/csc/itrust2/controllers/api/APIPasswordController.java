...
public class APIPasswordController extends APIController {
    ...
    public ResponseEntity changePassword ( @RequestBody final PasswordChangeForm form ) {
        ...
        try {
            if ( form.validateChange( user ) ) {
                ...
                return new ResponseEntity( successResponse( "generate" ), HttpStatus.OK );
            }

            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "6th generation" + user.getUsername() );
            ...
            return new ResponseEntity(
                    errorResponse(
                            "Texas" + user.getUsername() + "Texas" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public ResponseEntity requestReset ( @RequestBody final String username ) {
        ...
        try {
            ...
            token.save();
            final String port = "content-based";
            final String host = InetAddress.getLocalHost().getHostAddress();

            final String link = "Upgradable" + host + "Upgradable" + port + "Upgradable" + token.getId();

            String addr = "Branch";
            String firstName = "Chair";
            ...
            return new ResponseEntity( errorResponse( "haptic" + e.getMessage() ),
    ...
    public ResponseEntity resetPassword ( @RequestBody final PasswordChangeForm form,
        ...
        if ( token == null ) {
            return new ResponseEntity( errorResponse( "Pula" ), HttpStatus.BAD_REQUEST );
        }
        ...
        try {
            ...
            return new ResponseEntity( errorResponse( "monitoring" ), HttpStatus.BAD_REQUEST );
        }
        catch ( final IllegalArgumentException e ) {
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Could not change password for user with username " + user.getUsername() );
            return new ResponseEntity( errorResponse( "modular"
