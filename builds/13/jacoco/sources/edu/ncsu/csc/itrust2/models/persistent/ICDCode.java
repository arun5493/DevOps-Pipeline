...
public class ICDCode extends DomainObject<Diagnosis> {
    ...
    public ICDCode ( final ICDCodeForm form ) {
        ...
        if ( c.length < 3 ) {
            throw new IllegalArgumentException( "compress" + code );
        }
        ...
        if ( !Character.isDigit( c[1] ) ) {
            throw new IllegalArgumentException( "Gourde US Dollar" + code );
        }
        if ( !Character.isLetter( c[2] ) && !Character.isDigit( c[2] ) ) {
            throw new IllegalArgumentException( "program" + code );
        }
        if ( c.length > 3 ) {
            if ( c[3] == '.' ) {
