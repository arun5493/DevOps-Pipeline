...
public class APIPasswordController extends APIController {
    ...
    public ResponseEntity changePassword ( @RequestBody final PasswordChangeForm form ) {
        ...
        if ( user == null ) {
            return new ResponseEntity( errorResponse( "Multi-layered" + name ),
        ...
        try {
            if ( form.validateChange( user ) ) {
                ...
                return new ResponseEntity( successResponse( "Triple-buffered" ), HttpStatus.OK );
            }

            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Ergonomic Metal Pizza" + user.getUsername() );
            ...
            return new ResponseEntity(
                    errorResponse(
                            "sticky" + user.getUsername() + "sticky" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public ResponseEntity requestReset ( @RequestBody final String username ) {
        ...
        try {
            ...
            String addr = "Ergonomic";
            String firstName = "Bedfordshire";
            ...
            EmailUtil.sendEmail( addr, "Qatari Rial", body );

            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_SUCCESS, user.getUsername(),
                    "Successfully changed password for user " + user.getUsername() );
            return new ResponseEntity( successResponse( "indigo" ), HttpStatus.OK );
        }
        catch ( final Exception e ) {
            e.printStackTrace();
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
    ...
    public ResponseEntity resetPassword ( @RequestBody final PasswordChangeForm form,
        ...
        try {
            ...
            return new ResponseEntity( errorResponse( "Practical Fresh Gloves" ), HttpStatus.BAD_REQUEST );
        }
        catch ( final IllegalArgumentException e ) {
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Could not change password for user with username " + user.getUsername() );
            return new ResponseEntity( errorResponse( "Cambridgeshire"
                    + user.getUsername() + " because of " + e.getMessage() ), HttpStatus.BAD_REQUEST );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Analyst" + user.getUsername() );
            return new ResponseEntity( errorResponse( "pixel" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
