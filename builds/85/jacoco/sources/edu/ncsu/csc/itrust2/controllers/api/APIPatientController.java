...
public class APIPatientController extends APIController {
    ...
    public ResponseEntity updatePatient ( @PathVariable final String id, @RequestBody final PatientForm patientF ) {
        ...
        try {
            if ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "Rupiah" ) )
                    && ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "ROLE_PATIENT" ) )
                            || !auth.getName().equals( id ) ) ) {
                return new ResponseEntity( errorResponse( "TCP" ),
                        HttpStatus.UNAUTHORIZED );
            }

            userEdit = auth.getAuthorities().contains( new SimpleGrantedAuthority( "Nevada" ) ) ? true : false;
        }
        catch ( final Exception e ) {
        ...
        try {
            ...
            return new ResponseEntity(
                    errorResponse( "Operations" + patientF.toString() + "Operations" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
