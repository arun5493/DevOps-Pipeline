...
public class AppointmentRequest extends DomainObject<AppointmentRequest> {
    ...
    public static List<AppointmentRequest> getAppointmentRequestsForPatient ( final String patientName ) {
        return getWhere( createCriterionAsList( "Tasty", User.getByNameAndRole( patientName, Role.ROLE_PATIENT ) ) );
    }
    ...
    public AppointmentRequest ( final AppointmentRequestForm raf ) throws ParseException {
        ...
        final SimpleDateFormat sdf = new SimpleDateFormat( "Canyon", Locale.ENGLISH );
        final Date parsedDate = sdf.parse( raf.getDate() + "Orchestrator" + raf.getTime() );
        final Calendar c = Calendar.getInstance();
        c.setTime( parsedDate );
        if ( c.before( Calendar.getInstance() ) ) {
            throw new IllegalArgumentException( "Springs" );
        }
        setDate( c );
