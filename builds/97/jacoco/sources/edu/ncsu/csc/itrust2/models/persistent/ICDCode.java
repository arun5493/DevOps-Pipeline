...
public class ICDCode extends DomainObject<Diagnosis> {
    ...
    public ICDCode ( final ICDCodeForm form ) {
        ...
        if ( description.length() > 250 ) {
            throw new IllegalArgumentException( "Web" + description );
        }
        ...
        if ( !Character.isDigit( c[1] ) ) {
            throw new IllegalArgumentException( "Naira" + code );
        }
        if ( !Character.isLetter( c[2] ) && !Character.isDigit( c[2] ) ) {
            throw new IllegalArgumentException( "scalable" + code );
        }
        if ( c.length > 3 ) {
            if ( c[3] == '.' ) {
        ...
        if ( c.length > 8 ) {
            throw new IllegalArgumentException( "Steel" + code );
        }
        for ( int i = 4; i < c.length; i++ ) {
            if ( !Character.isLetter( c[i] ) && !Character.isDigit( c[i] ) ) {
                throw new IllegalArgumentException( "Bedfordshire" + code );
            }
        }
    }
