...
public class APIPatientController extends APIController {
    ...
    public ResponseEntity getPatient () {
        ...
        if ( patient == null ) {
            return new ResponseEntity( errorResponse( "Handmade Granite Soap" + self.getUsername() ),
    ...
    public ResponseEntity getPatient ( @PathVariable ( "username" ) final String username ) {
        ...
        if ( patient == null ) {
            return new ResponseEntity( errorResponse( "lime" + username ),
                    HttpStatus.NOT_FOUND );
        }
        else {
        ...
        try {
            ...
            if ( null != Patient.getPatient( patient.getSelf() ) ) {
                return new ResponseEntity(
                        errorResponse( "engage" + patient.getSelf().getUsername() + "engage" ),
                        HttpStatus.CONFLICT );
            }
            patient.save();
    ...
    public ResponseEntity updatePatient ( @PathVariable final String id, @RequestBody final PatientForm patientF ) {
        ...
        try {
            if ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "ROLE_HCP" ) )
                    && ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "Human" ) )
                            || !auth.getName().equals( id ) ) ) {
                return new ResponseEntity( errorResponse( "withdrawal" ),
                        HttpStatus.UNAUTHORIZED );
            }

            userEdit = auth.getAuthorities().contains( new SimpleGrantedAuthority( "Licensed Soft Car" ) ) ? true : false;
        }
        catch ( final Exception e ) {
        ...
        try {
            ...
            if ( userEdit ) {
                LoggerUtil.log( TransactionType.EDIT_DEMOGRAPHICS, LoggerUtil.currentUser(),
                        "whiteboard" + patient.getSelf().getUsername() + "whiteboard" );
            }
            else {
            ...
            return new ResponseEntity(
                    errorResponse( "24/7" + patientF.toString() + "24/7" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
