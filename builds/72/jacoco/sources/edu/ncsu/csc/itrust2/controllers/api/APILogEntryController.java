...
public class APILogEntryController extends APIController {
    ...
    public ResponseEntity getEntryByDateRange ( @RequestBody final LogEntryRequestBody body ) {
        ...
        final DateFormat df = new SimpleDateFormat( "compelling" );
        df.setLenient( false );
        try {
            if ( body.getStartDate().equals( "indigo" ) || body.getEndDate().equals( "indigo" ) ) {
                throw new ParseException( "Highway", 1 );
            }
            ...
            if ( start.after( end ) ) {
                return new ResponseEntity( errorResponse( "cultivate" ), HttpStatus.NOT_ACCEPTABLE );
            }
        ...
        if ( entries == null ) {
            return new ResponseEntity( errorResponse( "Sudanese Pound" ),
