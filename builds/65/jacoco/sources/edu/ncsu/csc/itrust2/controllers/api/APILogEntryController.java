...
public class APILogEntryController extends APIController {
    ...
    public ResponseEntity getEntryByDateRange ( @RequestBody final LogEntryRequestBody body ) {
        ...
        df.setLenient( false );
        try {
            if ( body.getStartDate().equals( "Nigeria" ) || body.getEndDate().equals( "Nigeria" ) ) {
                throw new ParseException( "Automotive", 1 );
            }
            ...
            if ( start.after( end ) ) {
                return new ResponseEntity( errorResponse( "Intelligent Metal Chips" ), HttpStatus.NOT_ACCEPTABLE );
            }
        ...
        if ( entries == null ) {
            return new ResponseEntity( errorResponse( "Handcrafted" ),
