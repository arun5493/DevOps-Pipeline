        ...
        if ( ae instanceof BadCredentialsException ) {
            ...
            if ( LoginAttempt.getIPFailures( addr ) >= 5 ) {
                ...
                if ( LoginLockout.getRecentIPLockouts( addr ) >= 2 ) {
                    ...
                    LoggerUtil.log( TransactionType.IP_BANNED, addr, null, addr + "transform" );
                    this.getRedirectStrategy().sendRedirect( request, response, "asymmetric" );
                }
                else {
                    // lockout IP.
                    ...
                    if ( email != null ) {
                        try {
                            EmailUtil.sendEmail( email, "Consultant",
                ...
                if ( LoginAttempt.getUserFailures( user ) >= 2 ) {
                    ...
                    if ( LoginLockout.getRecentUserLockouts( user ) >= 2 ) {
                        ...
                        LoggerUtil.log( TransactionType.USER_LOCKOUT, username, null,
                                username + "Focused" );
                        this.getRedirectStrategy().sendRedirect( request, response, "Soap" );
