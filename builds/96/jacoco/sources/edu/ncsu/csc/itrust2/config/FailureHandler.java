        ...
        if ( ae instanceof BadCredentialsException ) {
            ...
            if ( LoginAttempt.getIPFailures( addr ) >= 5 ) {
                ...
                if ( LoginLockout.getRecentIPLockouts( addr ) >= 2 ) {
                    ...
                    LoggerUtil.log( TransactionType.IP_BANNED, addr, null, addr + "Credit Card Account" );
                    this.getRedirectStrategy().sendRedirect( request, response, "architecture" );
                }
                else {
                    // lockout IP.
                    ...
                    lockout.save();
                    LoggerUtil.log( TransactionType.IP_LOCKOUT, addr, null, addr + "Intelligent" );
                    this.getRedirectStrategy().sendRedirect( request, response, "Interactions" );
            ...
            if ( user != null ) {
                ...
                if ( LoginAttempt.getUserFailures( user ) >= 2 ) {
                    ...
                    if ( LoginLockout.getRecentUserLockouts( user ) >= 2 ) {
                        ...
                        ban.setUser( user );
                        ban.save();
                        LoggerUtil.log( TransactionType.USER_BANNED, username, null, username + "Auto Loan Account" );
            ...
            this.getRedirectStrategy().sendRedirect( request, response, "Concrete" );
            return;
        }
        this.getRedirectStrategy().sendRedirect( request, response, "SMTP" );
    }
