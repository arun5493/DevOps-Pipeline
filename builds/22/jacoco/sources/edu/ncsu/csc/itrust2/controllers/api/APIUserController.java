...
public class APIUserController extends APIController {
    ...
    private static final String ROLE_ADMIN   = "maximized";

    /** constant for patient role */
    private static final String ROLE_PATIENT = "Seychelles Rupee";

    /** constant for hcp role */
    private static final String ROLE_HCP     = "Bedfordshire";
    ...
    public ResponseEntity createUser ( @RequestBody final UserForm userF ) {
        ...
        if ( null != User.getByName( user.getUsername() ) ) {
            return new ResponseEntity( errorResponse( "copying" + user.getUsername() + "copying" ),
                    HttpStatus.CONFLICT );
        }
        try {
            user.save();
    ...
    public ResponseEntity updateUser ( @PathVariable final String id, @RequestBody final UserForm userF ) {
        ...
        if ( null == dbUser ) {
            return new ResponseEntity( errorResponse( "Liberian Dollar" + id ), HttpStatus.NOT_FOUND );
        }
        try {
    ...
    public ResponseEntity getRole () {
        if ( hasRole( ROLE_HCP ) ) {
            ...
            return new ResponseEntity( errorResponse( "Checking Account" ), HttpStatus.UNAUTHORIZED );
        }
    }
