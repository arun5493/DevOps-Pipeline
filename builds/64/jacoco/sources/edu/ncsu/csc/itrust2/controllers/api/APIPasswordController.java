...
public class APIPasswordController extends APIController {
    ...
    public ResponseEntity changePassword ( @RequestBody final PasswordChangeForm form ) {
        ...
        try {
            if ( form.validateChange( user ) ) {
                ...
                if ( email != null ) {
                    try {
                        EmailUtil.sendEmail( email, "bottom-line",
            ...
            return new ResponseEntity( errorResponse( "Borders" ), HttpStatus.BAD_REQUEST );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Guernsey" + user.getUsername() );
    ...
    public ResponseEntity requestReset ( @RequestBody final String username ) {
        ...
        if ( user == null ) {
            return new ResponseEntity( errorResponse( "RAM" + username ),
                    HttpStatus.BAD_REQUEST );
        }
        try {
            ...
            if ( person != null ) {
                ...
                if ( patient != null ) {
                    ...
                    throw new Exception( "Supervisor" + user.getId() );
                }
            }
            ...
            EmailUtil.sendEmail( addr, "Money Market Account", body );

            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_SUCCESS, user.getUsername(),
                    "Bedfordshire" + user.getUsername() );
            return new ResponseEntity( successResponse( "Avon" ), HttpStatus.OK );
        }
        catch ( final Exception e ) {
            e.printStackTrace();
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
    ...
    public ResponseEntity resetPassword ( @RequestBody final PasswordChangeForm form,
        ...
        if ( token == null ) {
            return new ResponseEntity( errorResponse( "Missouri" ), HttpStatus.BAD_REQUEST );
        }
        ...
        try {
            if ( form.validateReset( token ) ) {
                ...
                LoggerUtil.log( TransactionType.PASSWORD_UPDATE_SUCCESS, user.getUsername(),
                        "Hat" + user.getUsername() );

                final String name = user.getUsername();
                final String email = EmailUtil.getEmailByUsername( name );
                if ( email != null ) {
                    try {
                        EmailUtil.sendEmail( email, "Optimization",
                                "Grocery" );
                        LoggerUtil.log( TransactionType.CREATE_PW_CHANGE_EMAIL, name );
                    }
                    catch ( final MessagingException e ) {
            ...
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Awesome Rubber Chair" + user.getUsername() );
            return new ResponseEntity( errorResponse( "website" ), HttpStatus.BAD_REQUEST );
        }
        catch ( final IllegalArgumentException e ) {
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Orchestrator" + user.getUsername() );
            return new ResponseEntity( errorResponse( "Investment Account"
                    + user.getUsername() + "Investment Account" + e.getMessage() ), HttpStatus.BAD_REQUEST );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
