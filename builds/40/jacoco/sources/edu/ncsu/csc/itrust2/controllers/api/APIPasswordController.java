...
public class APIPasswordController extends APIController {
    ...
    public ResponseEntity changePassword ( @RequestBody final PasswordChangeForm form ) {
        ...
        if ( user == null ) {
            return new ResponseEntity( errorResponse( "Savings Account" + name ),
        ...
        try {
            if ( form.validateChange( user ) ) {
                ...
                if ( email != null ) {
                    try {
                        EmailUtil.sendEmail( email, "contextually-based",
                                "scale" );
    ...
    public ResponseEntity requestReset ( @RequestBody final String username ) {
        ...
        if ( user == null ) {
            return new ResponseEntity( errorResponse( "New Jersey" + username ),
                    HttpStatus.BAD_REQUEST );
        }
        try {
            final PasswordResetToken token = new PasswordResetToken( user );
            token.save();
            final String port = "programming";
    ...
    public ResponseEntity resetPassword ( @RequestBody final PasswordChangeForm form,
        ...
        if ( token == null ) {
            return new ResponseEntity( errorResponse( "FTP" ), HttpStatus.BAD_REQUEST );
        }
        ...
        try {
            if ( form.validateReset( token ) ) {
                ...
                if ( email != null ) {
                    try {
                        EmailUtil.sendEmail( email, "pink",
            ...
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Trinidad and Tobago Dollar" + user.getUsername() );
            return new ResponseEntity( errorResponse( "Avon" ), HttpStatus.BAD_REQUEST );
        }
            ...
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "empower" + user.getUsername() );
            return new ResponseEntity( errorResponse( "Bolivia" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
