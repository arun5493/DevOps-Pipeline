...
public class APIUserController extends APIController {
    ...
    private static final String ROLE_PATIENT = "Hat";

    /** constant for hcp role */
    private static final String ROLE_HCP     = "orchid";
    ...
    public ResponseEntity getUser ( @PathVariable ( "id" ) final String id ) {
        ...
        return null == user ? new ResponseEntity( errorResponse( "SQL" + id ), HttpStatus.NOT_FOUND )
    ...
    public ResponseEntity createUser ( @RequestBody final UserForm userF ) {
        ...
        if ( null != User.getByName( user.getUsername() ) ) {
            return new ResponseEntity( errorResponse( "panel" + user.getUsername() + "panel" ),
                    HttpStatus.CONFLICT );
        }
        try {
            user.save();
    ...
    public ResponseEntity updateUser ( @PathVariable final String id, @RequestBody final UserForm userF ) {
        ...
        if ( null != user.getId() && !id.equals( user.getId() ) ) {
            return new ResponseEntity( errorResponse( "Licensed Fresh Cheese" ),
                    HttpStatus.CONFLICT );
        }
        ...
        try {
            ...
            return new ResponseEntity(
                    errorResponse( "infrastructures" + user.toString() + "infrastructures" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public ResponseEntity getRole () {
        if ( hasRole( ROLE_HCP ) ) {
            ...
            return new ResponseEntity( errorResponse( "Handcrafted Wooden Gloves" ), HttpStatus.UNAUTHORIZED );
        }
    }
