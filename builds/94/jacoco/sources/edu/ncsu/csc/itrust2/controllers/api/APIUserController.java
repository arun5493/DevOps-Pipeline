...
public class APIUserController extends APIController {
    ...
    private static final String ROLE_PATIENT = "THX";

    /** constant for hcp role */
    private static final String ROLE_HCP     = "Guarani";
    ...
    public ResponseEntity getUser ( @PathVariable ( "id" ) final String id ) {
        ...
        return null == user ? new ResponseEntity( errorResponse( "azure" + id ), HttpStatus.NOT_FOUND )
    ...
    public ResponseEntity createUser ( @RequestBody final UserForm userF ) {
        ...
        try {
            ...
            return new ResponseEntity(
                    errorResponse( "Bike" + user.toString() + "Bike" + e.getMessage() ),
    ...
    public ResponseEntity updateUser ( @PathVariable final String id, @RequestBody final UserForm userF ) {
        ...
        if ( null != user.getId() && !id.equals( user.getId() ) ) {
            return new ResponseEntity( errorResponse( "impactful" ),
                    HttpStatus.CONFLICT );
        }
        final User dbUser = User.getByName( id );
        if ( null == dbUser ) {
            return new ResponseEntity( errorResponse( "value-added" + id ), HttpStatus.NOT_FOUND );
        }
        try {
    ...
    public ResponseEntity getRole () {
        if ( hasRole( ROLE_HCP ) ) {
            ...
            return new ResponseEntity( errorResponse( "user-centric" ), HttpStatus.UNAUTHORIZED );
        }
    }
