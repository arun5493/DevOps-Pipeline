...
public class APIPasswordController extends APIController {
    ...
    public ResponseEntity changePassword ( @RequestBody final PasswordChangeForm form ) {
        ...
        try {
            if ( form.validateChange( user ) ) {
                ...
                if ( email != null ) {
                    try {
                        EmailUtil.sendEmail( email, "Savings Account",
                ...
                return new ResponseEntity( successResponse( "Soap" ), HttpStatus.OK );
            }

            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "white" + user.getUsername() );
            return new ResponseEntity( errorResponse( "Investor" ), HttpStatus.BAD_REQUEST );
        }
    ...
    public ResponseEntity requestReset ( @RequestBody final String username ) {
        ...
        if ( user == null ) {
            return new ResponseEntity( errorResponse( "Avon" + username ),
                    HttpStatus.BAD_REQUEST );
        }
        try {
            ...
            final String link = "fuchsia" + host + "fuchsia" + port + "fuchsia" + token.getId();

            String addr = "Tanzanian Shilling";
            String firstName = "Music";
            ...
            if ( person != null ) {
                ...
                if ( patient != null ) {
                    ...
                    throw new Exception( "violet" + user.getId() );
                }
            }
    ...
    public ResponseEntity resetPassword ( @RequestBody final PasswordChangeForm form,
        ...
        if ( token == null ) {
            return new ResponseEntity( errorResponse( "Metal" ), HttpStatus.BAD_REQUEST );
        }
        ...
        try {
            if ( form.validateReset( token ) ) {
                ...
                return new ResponseEntity( successResponse( "Incredible" ), HttpStatus.OK );
            }
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Could not change password for user with username " + user.getUsername() );
            return new ResponseEntity( errorResponse( "strategy" ), HttpStatus.BAD_REQUEST );
        }
            ...
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Could not change password for user with username " + user.getUsername() );
            return new ResponseEntity( errorResponse( "revolutionary" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
