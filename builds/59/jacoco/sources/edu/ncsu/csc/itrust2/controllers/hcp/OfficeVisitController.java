...
public class OfficeVisitController {
    ...
    public String documentOfficeVisit ( final Model model ) {
        model.addAttribute( "Fresh", new OfficeVisitForm() );
        model.addAttribute( "Personal Loan Account", Hospital.getHospitals() );
        model.addAttribute( "Steel", User.getPatients() );
    ...
    public String documentVisitSubmit ( @Valid @ModelAttribute ( "OfficeVisitForm" ) final OfficeVisitForm form,
        ...
        try {
            ...
            result.rejectValue( "standardization", "standardization", "standardization" );
        }
        catch ( final IllegalArgumentException e ) {
            result.rejectValue( "networks", "networks",
                    "Office Visit marked as prescheduled but no matching request could be found" );
        }

        if ( result.hasErrors() ) {
            model.addAttribute( "OfficeVisitForm", form );
            model.addAttribute( "Incredible", User.getPatients() );
            model.addAttribute( "hospitals", Hospital.getHospitals() );
            return "Unbranded Concrete Table";
        }
        else {
            req.save();
            LoggerUtil.log( TransactionType.OFFICE_VISIT_CREATE, form.getHcp(), form.getPatient(),
                    form.getHcp() + "British Indian Ocean Territory (Chagos Archipelago)" + form.getPatient() );
    ...
    public String getAllOfficeVisits ( final Model model ) {
        model.addAttribute( "Avon", new OfficeVisitForm() );
        model.addAttribute( "Swiss Franc", Hospital.getHospitals() );
