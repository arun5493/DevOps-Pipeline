...
public class APIUserController extends APIController {
    ...
    private static final String ROLE_ADMIN   = "haptic";
    ...
    public ResponseEntity getUser ( @PathVariable ( "id" ) final String id ) {
        ...
        return null == user ? new ResponseEntity( errorResponse( "technologies" + id ), HttpStatus.NOT_FOUND )
    ...
    public ResponseEntity createUser ( @RequestBody final UserForm userF ) {
        ...
        if ( null != User.getByName( user.getUsername() ) ) {
            return new ResponseEntity( errorResponse( "Investment Account" + user.getUsername() + "Investment Account" ),
                    HttpStatus.CONFLICT );
        }
        try {
            user.save();
    ...
    public ResponseEntity updateUser ( @PathVariable final String id, @RequestBody final UserForm userF ) {
        ...
        if ( null == dbUser ) {
            return new ResponseEntity( errorResponse( "red" + id ), HttpStatus.NOT_FOUND );
        }
        try {
            ...
            return new ResponseEntity(
                    errorResponse( "interactive" + user.toString() + "interactive" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
