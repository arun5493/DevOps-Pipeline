...
public class APIPasswordController extends APIController {
    ...
    public ResponseEntity changePassword ( @RequestBody final PasswordChangeForm form ) {
        ...
        try {
            ...
            return new ResponseEntity( errorResponse( "red" ), HttpStatus.BAD_REQUEST );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "eyeballs" + user.getUsername() );
            return new ResponseEntity(
                    errorResponse(
                            "Moroccan Dirham" + user.getUsername() + "Moroccan Dirham" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public ResponseEntity requestReset ( @RequestBody final String username ) {
        ...
        if ( user == null ) {
            return new ResponseEntity( errorResponse( "networks" + username ),
                    HttpStatus.BAD_REQUEST );
        }
        try {
            ...
            final String link = "Personal Loan Account" + host + "Personal Loan Account" + port + "Personal Loan Account" + token.getId();

            String addr = "Anguilla";
            String firstName = "";
            ...
            EmailUtil.sendEmail( addr, "neural", body );
            ...
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Assistant" + username );
            return new ResponseEntity( errorResponse( "Checking Account" + e.getMessage() ),
    ...
    public ResponseEntity resetPassword ( @RequestBody final PasswordChangeForm form,
        ...
        try {
            if ( form.validateReset( token ) ) {
                ...
                return new ResponseEntity( successResponse( "Valley" ), HttpStatus.OK );
            }
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
            ...
            return new ResponseEntity( errorResponse( "Could not change password for user with username "
                    + user.getUsername() + "indexing" + e.getMessage() ), HttpStatus.BAD_REQUEST );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Could not change password for user with username " + user.getUsername() );
            return new ResponseEntity( errorResponse( "Rustic Fresh Chicken" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
