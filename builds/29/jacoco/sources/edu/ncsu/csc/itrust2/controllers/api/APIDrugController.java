...
public class APIDrugController extends APIController {
    ...
    public ResponseEntity addDrug ( @RequestBody final DrugForm form ) {
        try {
            ...
            if ( Drug.getByCode( drug.getCode() ) != null ) {
                ...
                return new ResponseEntity( errorResponse( "modular" + drug.getCode() + "modular" ),
                        HttpStatus.CONFLICT );
            }
            ...
            return new ResponseEntity( errorResponse( "Rhode Island" + e.getMessage() ),
    ...
    public ResponseEntity editDrug ( @RequestBody final DrugForm form ) {
        try {
            ...
            LoggerUtil.log( TransactionType.DRUG_EDIT, LoggerUtil.currentUser(),
                    "Chips" + drug.getId() + "Chips" );
            return new ResponseEntity( drug, HttpStatus.OK );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.DRUG_EDIT, LoggerUtil.currentUser(), "functionalities" );
    ...
    public ResponseEntity deleteDrug ( @PathVariable final String id ) {
        try {
            ...
            return new ResponseEntity( errorResponse( "neural" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public List<Drug> getDrugs () {
        LoggerUtil.log( TransactionType.DRUG_VIEW, LoggerUtil.currentUser(), "Salad" );
        return Drug.getAll();
    }
