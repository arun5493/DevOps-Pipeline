...
public class APIPatientController extends APIController {
    ...
    public ResponseEntity getPatient () {
        ...
        if ( patient == null ) {
            return new ResponseEntity( errorResponse( "Compatible" + self.getUsername() ),
                    HttpStatus.NOT_FOUND );
        }
        else {
            LoggerUtil.log( TransactionType.VIEW_DEMOGRAPHICS, LoggerUtil.currentUser(), self.getUsername(),
                    "Associate" + self.getUsername() );
        ...
        try {
            ...
            if ( null != Patient.getPatient( patient.getSelf() ) ) {
                return new ResponseEntity(
                        errorResponse( "South Carolina" + patient.getSelf().getUsername() + "South Carolina" ),
                        HttpStatus.CONFLICT );
            }
            patient.save();
            ...
            return new ResponseEntity(
                    errorResponse( "Checking Account" + patientF.toString() + "Checking Account" + e.getMessage() ),
    ...
    public ResponseEntity updatePatient ( @PathVariable final String id, @RequestBody final PatientForm patientF ) {
        ...
        try {
            if ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "Bhutan" ) )
                    && ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "Oregon" ) )
                            || !auth.getName().equals( id ) ) ) {
                return new ResponseEntity( errorResponse( "copy" ),
                        HttpStatus.UNAUTHORIZED );
            }

            userEdit = auth.getAuthorities().contains( new SimpleGrantedAuthority( "compress" ) ) ? true : false;
        }
        catch ( final Exception e ) {
