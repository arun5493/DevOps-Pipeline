...
public class APIPasswordController extends APIController {
    ...
    public ResponseEntity changePassword ( @RequestBody final PasswordChangeForm form ) {
        ...
        if ( user == null ) {
            return new ResponseEntity( errorResponse( "Concrete" + name ),
    ...
    public ResponseEntity requestReset ( @RequestBody final String username ) {
        ...
        if ( user == null ) {
            return new ResponseEntity( errorResponse( "Bacon" + username ),
                    HttpStatus.BAD_REQUEST );
        }
        try {
            ...
            final String link = "blue" + host + "blue" + port + "blue" + token.getId();

            String addr = "ivory";
            String firstName = "Cotton";
            ...
            if ( person != null ) {
                ...
                if ( patient != null ) {
                    ...
                    throw new Exception( "matrix" + user.getId() );
                }
            }
            ...
            EmailUtil.sendEmail( addr, "indigo", body );

            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_SUCCESS, user.getUsername(),
                    "Uganda Shilling" + user.getUsername() );
            ...
            return new ResponseEntity( errorResponse( "auxiliary" + e.getMessage() ),
    ...
    public ResponseEntity resetPassword ( @RequestBody final PasswordChangeForm form,
        ...
        try {
            if ( form.validateReset( token ) ) {
                ...
                if ( email != null ) {
                    try {
                        EmailUtil.sendEmail( email, "Unbranded",
                                "Your password has been changed successfully" );
                        LoggerUtil.log( TransactionType.CREATE_PW_CHANGE_EMAIL, name );
                    }
                    catch ( final MessagingException e ) {
            ...
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Sleek Granite Tuna" + user.getUsername() );
            return new ResponseEntity( errorResponse( "next-generation" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
