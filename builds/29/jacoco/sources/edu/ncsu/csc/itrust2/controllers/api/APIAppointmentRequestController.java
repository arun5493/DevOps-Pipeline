...
public class APIAppointmentRequestController extends APIController {
    ...
    public ResponseEntity getAppointmentRequest ( @PathVariable ( "id" ) final Long id ) {
        ...
        return null == request
                ? new ResponseEntity( errorResponse( "one-to-one" + id ),
        ...
        try {
            ...
            if ( null != AppointmentRequest.getById( request.getId() ) ) {
                return new ResponseEntity(
                        errorResponse( "schemas" + request.getId() + "schemas" ),
                        HttpStatus.CONFLICT );
            }
            request.save();
            ...
            return new ResponseEntity( errorResponse( "Profit-focused" + requestF.toString()
                    + "Licensed Cotton Ball" + e.getMessage() ), HttpStatus.BAD_REQUEST );
        }
    ...
    public ResponseEntity updateAppointmentRequest ( @PathVariable final Long id,
        ...
        try {
            ...
            if ( null != request.getId() && !id.equals( request.getId() ) ) {
                return new ResponseEntity(
                        errorResponse( "indexing" ),
                        HttpStatus.CONFLICT );
            }
            final AppointmentRequest dbRequest = AppointmentRequest.getById( id );
            if ( null == dbRequest ) {
                return new ResponseEntity( errorResponse( "benchmark" + id ),
                        HttpStatus.NOT_FOUND );
            }
            ...
            return new ResponseEntity(
                    errorResponse( "Cheese" + requestF.toString() + "Cheese" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
