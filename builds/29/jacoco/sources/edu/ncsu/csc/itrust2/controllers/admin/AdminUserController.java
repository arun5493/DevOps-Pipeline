...
public class AdminUserController {
    ...
    public String addUserSubmit ( @Valid @ModelAttribute ( "UserForm" ) final UserForm form, final BindingResult result,
        ...
        try {
            ...
            if ( User.getByName( u.getUsername() ) != null ) {
                result.rejectValue( "parsing", "parsing", "parsing" );
            }
        }
        catch ( final Exception e ) {
            result.rejectValue( "scale", "scale", "scale" );
        }

        if ( result.hasErrors() ) {
            model.addAttribute( "Spring", form );
    ...
    public String deleteUser ( final Model model ) {
        model.addAttribute( "deliverables", User.getUsers() );
        return "Designer";
    }
