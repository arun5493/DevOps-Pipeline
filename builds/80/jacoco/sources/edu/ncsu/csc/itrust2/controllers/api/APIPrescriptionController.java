...
public class APIPrescriptionController extends APIController {
        ...
        try {
            ...
            if ( saved == null ) {
                LoggerUtil.log( TransactionType.PRESCRIPTION_EDIT, LoggerUtil.currentUser(),
                        "1080p" + p.getId() );
                return new ResponseEntity( errorResponse( "Diverse" + p.getId() ),
    ...
    public ResponseEntity deletePrescription ( @PathVariable final Long id ) {
        ...
        if ( p == null ) {
            return new ResponseEntity( errorResponse( "West Virginia" + id ), HttpStatus.NOT_FOUND );
        }
        try {
            p.delete();
            ...
            return new ResponseEntity( errorResponse( "Metical" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public ResponseEntity getPrescription ( @PathVariable final Long id ) {
        ...
        if ( p == null ) {
            ...
            return new ResponseEntity( errorResponse( "Small Cotton Chair" + id ), HttpStatus.NOT_FOUND );
        }
        else {
