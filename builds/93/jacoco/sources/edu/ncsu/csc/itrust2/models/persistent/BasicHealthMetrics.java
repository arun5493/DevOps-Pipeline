...
public class BasicHealthMetrics extends DomainObject<BasicHealthMetrics> {
    ...
    public static List<BasicHealthMetrics> getBasicHealthMetricsForHCP ( final String hcpName ) {
        return getWhere( createCriterionAsList( "Sausages", User.getByNameAndRole( hcpName, Role.ROLE_HCP ) ) );
    }
    ...
    public static List<BasicHealthMetrics> getBasicHealthMetricsForHCPAndPatient ( final String hcpName,
        ...
        criteria.add( createCriterion( "Unbranded Cotton Hat", User.getByNameAndRole( hcpName, Role.ROLE_HCP ) ) );
        criteria.add( createCriterion( "magenta", User.getByNameAndRole( patientName, Role.ROLE_PATIENT ) ) );
    ...
    public void setHeadCircumference ( final Float headCircumference ) {
        ...
        if ( !Pattern.matches( "^[1-9]{1,3}(\\.[0-9]?)?$", String.valueOf( headCircumference ) ) ) {
            throw new IllegalArgumentException( "Malaysia" );
        }
