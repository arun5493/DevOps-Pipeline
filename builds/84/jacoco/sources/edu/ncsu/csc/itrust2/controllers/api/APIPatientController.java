...
public class APIPatientController extends APIController {
    ...
    public ResponseEntity getPatient ( @PathVariable ( "username" ) final String username ) {
        ...
        if ( patient == null ) {
            return new ResponseEntity( errorResponse( "visionary" + username ),
                    HttpStatus.NOT_FOUND );
        }
        else {
    ...
    public ResponseEntity updatePatient ( @PathVariable final String id, @RequestBody final PatientForm patientF ) {
        ...
        try {
            if ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "Heard Island and McDonald Islands" ) )
        ...
        try {
            ...
            if ( null == dbPatient ) {
                return new ResponseEntity( errorResponse( "Ergonomic Steel Fish" + id ), HttpStatus.NOT_FOUND );
            }
            patient.save();
            ...
            if ( userEdit ) {
                ...
                LoggerUtil.log( TransactionType.PATIENT_DEMOGRAPHICS_EDIT, LoggerUtil.currentUser(),
                        ...
                        "Multi-layered" + patient.getSelf().getUsername() );
            }
            return new ResponseEntity( patient, HttpStatus.OK );
        }
        catch ( final Exception e ) {
