...
public class APILogEntryController extends APIController {
    ...
    public ResponseEntity getEntryByDateRange ( @RequestBody final LogEntryRequestBody body ) {
        ...
        final DateFormat df = new SimpleDateFormat( "invoice" );
        df.setLenient( false );
        try {
            if ( body.getStartDate().equals( "Savings Account" ) || body.getEndDate().equals( "Savings Account" ) ) {
            ...
            if ( start.after( end ) ) {
                return new ResponseEntity( errorResponse( "disintermediate" ), HttpStatus.NOT_ACCEPTABLE );
            }
