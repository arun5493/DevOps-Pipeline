...
public class APIPasswordController extends APIController {
    ...
    public ResponseEntity changePassword ( @RequestBody final PasswordChangeForm form ) {
        ...
        if ( user == null ) {
            return new ResponseEntity( errorResponse( "PNG" + name ),
        ...
        try {
            if ( form.validateChange( user ) ) {
                ...
                if ( email != null ) {
                    try {
                        EmailUtil.sendEmail( email, "Coordinator",
                                "Decentralized" );
                ...
                return new ResponseEntity( successResponse( "driver" ), HttpStatus.OK );
            }
            ...
            return new ResponseEntity( errorResponse( "eyeballs" ), HttpStatus.BAD_REQUEST );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Personal Loan Account" + user.getUsername() );
            return new ResponseEntity(
                    errorResponse(
                            "California" + user.getUsername() + "California" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public ResponseEntity requestReset ( @RequestBody final String username ) {
        ...
        try {
            ...
            final String link = "Moroccan Dirham" + host + "Moroccan Dirham" + port + "Moroccan Dirham" + token.getId();
            ...
            return new ResponseEntity( successResponse( "South Carolina" ), HttpStatus.OK );
        }
        catch ( final Exception e ) {
            e.printStackTrace();
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "interface" + username );
            return new ResponseEntity( errorResponse( "bleeding-edge" + e.getMessage() ),
    ...
    public ResponseEntity resetPassword ( @RequestBody final PasswordChangeForm form,
        ...
        try {
            if ( form.validateReset( token ) ) {
                ...
                return new ResponseEntity( successResponse( "Handcrafted" ), HttpStatus.OK );
            }
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "SMTP" + user.getUsername() );
            return new ResponseEntity( errorResponse( "deliver" ), HttpStatus.BAD_REQUEST );
        }
        catch ( final IllegalArgumentException e ) {
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Could not change password for user with username " + user.getUsername() );
            return new ResponseEntity( errorResponse( "Checking Account"
                    + user.getUsername() + " because of " + e.getMessage() ), HttpStatus.BAD_REQUEST );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "multi-byte" + user.getUsername() );
            return new ResponseEntity( errorResponse( "Saudi Riyal" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
