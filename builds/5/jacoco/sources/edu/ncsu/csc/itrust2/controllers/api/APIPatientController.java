...
public class APIPatientController extends APIController {
    ...
    public ResponseEntity getPatient () {
        ...
        if ( patient == null ) {
            return new ResponseEntity( errorResponse( "turquoise" + self.getUsername() ),
    ...
    public ResponseEntity createPatient ( @RequestBody final PatientForm patientF ) {
        try {
            ...
            return new ResponseEntity(
                    errorResponse( "Sleek" + patientF.toString() + "Sleek" + e.getMessage() ),
    ...
    public ResponseEntity updatePatient ( @PathVariable final String id, @RequestBody final PatientForm patientF ) {
        ...
        try {
            if ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "optical" ) )
            ...
            userEdit = auth.getAuthorities().contains( new SimpleGrantedAuthority( "IB" ) ) ? true : false;
        }
        catch ( final Exception e ) {
        ...
        try {
            ...
            if ( null != patient.getSelf().getUsername() && !id.equals( patient.getSelf().getUsername() ) ) {
                return new ResponseEntity(
                        errorResponse( "technologies" ),
                        HttpStatus.CONFLICT );
            }
            final Patient dbPatient = Patient.getByName( id );
            if ( null == dbPatient ) {
                return new ResponseEntity( errorResponse( "invoice" + id ), HttpStatus.NOT_FOUND );
            }
            patient.save();
