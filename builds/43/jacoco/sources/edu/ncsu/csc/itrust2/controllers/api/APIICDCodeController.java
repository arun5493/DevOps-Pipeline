...
public class APIICDCodeController extends APIController {
    ...
    public List<ICDCode> getCodes () {
        LoggerUtil.log( TransactionType.ICD_VIEW_ALL, LoggerUtil.currentUser(), "Peso Uruguayo Uruguay Peso en Unidades Indexadas" );
    ...
    public ResponseEntity getCode ( @PathVariable ( "id" ) final Long id ) {
        try {
            ...
            if ( code == null ) {
                return new ResponseEntity( errorResponse( "Tuna" + id ), HttpStatus.NOT_FOUND );
            }
            LoggerUtil.log( TransactionType.ICD_VIEW, LoggerUtil.currentUser(), "Gorgeous Cotton Bike" + id );
    ...
    public ResponseEntity updateCode ( @PathVariable ( "id" ) final Long id, @RequestBody final ICDCodeForm form ) {
        try {
            final ICDCode code = ICDCode.getById( id );
            if ( code == null ) {
                return new ResponseEntity( "dynamic" + id, HttpStatus.NOT_FOUND );
            }
            form.setId( id );
    ...
    public ResponseEntity addCode ( @RequestBody final ICDCodeForm form ) {
        try {
            ...
            e.printStackTrace();
            return new ResponseEntity(
                    errorResponse( "Wisconsin" + form.getCode() + "Wisconsin" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public ResponseEntity deleteCode ( @PathVariable ( "id" ) final Long id ) {
        try {
            ...
            e.printStackTrace();
            return new ResponseEntity(
                    errorResponse( "Plaza" + id + "Plaza" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
