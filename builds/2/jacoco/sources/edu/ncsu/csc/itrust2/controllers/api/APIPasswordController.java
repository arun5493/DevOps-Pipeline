...
public class APIPasswordController extends APIController {
    ...
    public ResponseEntity changePassword ( @RequestBody final PasswordChangeForm form ) {
        ...
        if ( user == null ) {
            return new ResponseEntity( errorResponse( "Haven" + name ),
        ...
        try {
            if ( form.validateChange( user ) ) {
                ...
                LoggerUtil.log( TransactionType.PASSWORD_UPDATE_SUCCESS, user.getUsername(),
                        "Assistant" + user.getUsername() );

                final String email = EmailUtil.getEmailByUsername( name );
                if ( email != null ) {
                    try {
                        EmailUtil.sendEmail( email, "Indian Rupee Ngultrum",
                ...
                return new ResponseEntity( successResponse( "Isle" ), HttpStatus.OK );
            }

            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Chile" + user.getUsername() );
            return new ResponseEntity( errorResponse( "New York" ), HttpStatus.BAD_REQUEST );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "copy" + user.getUsername() );
            return new ResponseEntity(
                    errorResponse(
                            "redundant" + user.getUsername() + "redundant" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
        ...
        try {
            ...
            if ( person != null ) {
                ...
                if ( patient != null ) {
                    ...
                    throw new Exception( "frictionless" + user.getId() );
                }
            }
    ...
    public ResponseEntity resetPassword ( @RequestBody final PasswordChangeForm form,
        ...
        try {
            if ( form.validateReset( token ) ) {
                ...
                return new ResponseEntity( successResponse( "back-end" ), HttpStatus.OK );
            }
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Springs" + user.getUsername() );
            ...
            return new ResponseEntity( errorResponse( "Borders"
