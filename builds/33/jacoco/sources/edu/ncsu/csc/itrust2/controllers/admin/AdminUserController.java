...
public class AdminUserController {
    ...
    public String addUser ( final Model model ) {
        model.addAttribute( "Awesome Concrete Hat", new UserForm() );
    ...
    public String addUserSubmit ( @Valid @ModelAttribute ( "UserForm" ) final UserForm form, final BindingResult result,
        ...
        try {
            ...
            if ( User.getByName( u.getUsername() ) != null ) {
                result.rejectValue( "California", "California", "California" );
            }
        }
        catch ( final Exception e ) {
            result.rejectValue( "superstructure", "superstructure", "superstructure" );
        }

        if ( result.hasErrors() ) {
            model.addAttribute( "index", form );
            return "B2B";
        }
        else {
            u.save();
