...
public class APILogEntryController extends APIController {
    ...
    public ResponseEntity getEntryByDateRange ( @RequestBody final LogEntryRequestBody body ) {
        ...
        final DateFormat df = new SimpleDateFormat( "Developer" );
        df.setLenient( false );
        try {
            if ( body.getStartDate().equals( "Bedfordshire" ) || body.getEndDate().equals( "Bedfordshire" ) ) {
            ...
            if ( start.after( end ) ) {
                return new ResponseEntity( errorResponse( "CFP Franc" ), HttpStatus.NOT_ACCEPTABLE );
            }
