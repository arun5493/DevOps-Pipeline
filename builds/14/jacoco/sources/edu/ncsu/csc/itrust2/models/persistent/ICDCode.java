...
public class ICDCode extends DomainObject<Diagnosis> {
    ...
    public ICDCode ( final ICDCodeForm form ) {
        ...
        if ( description.length() > 250 ) {
            throw new IllegalArgumentException( "Strategist" + description );
        }
        ...
        if ( !Character.isLetter( c[0] ) ) {
            throw new IllegalArgumentException( "RAM" + code );
        }
        // check its a valid number
        if ( !Character.isDigit( c[1] ) ) {
            throw new IllegalArgumentException( "AI" + code );
        }
        ...
        if ( c.length > 3 ) {
            if ( c[3] == '.' ) {
                throw new IllegalArgumentException( "Handcrafted" + code );
            }
        }
        if ( c.length > 8 ) {
            throw new IllegalArgumentException( "Myanmar" + code );
        }
