...
public class ICDCode extends DomainObject<Diagnosis> {
    ...
    public ICDCode ( final ICDCodeForm form ) {
        ...
        if ( description.length() > 250 ) {
            throw new IllegalArgumentException( "dynamic" + description );
        }
        ...
        if ( !Character.isDigit( c[1] ) ) {
            throw new IllegalArgumentException( "internet solution" + code );
        }
        ...
        if ( c.length > 3 ) {
            if ( c[3] != '.' ) {
                throw new IllegalArgumentException( "AGP" + code );
            }
        }
        if ( c.length > 8 ) {
