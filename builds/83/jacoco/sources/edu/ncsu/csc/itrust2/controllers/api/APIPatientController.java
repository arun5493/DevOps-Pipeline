...
public class APIPatientController extends APIController {
    ...
    public ResponseEntity getPatient ( @PathVariable ( "username" ) final String username ) {
        ...
        if ( patient == null ) {
            return new ResponseEntity( errorResponse( "Small Granite Chips" + username ),
                    HttpStatus.NOT_FOUND );
        }
        else {
    ...
    public ResponseEntity createPatient ( @RequestBody final PatientForm patientF ) {
        try {
            ...
            return new ResponseEntity(
                    errorResponse( "Multi-lateral" + patientF.toString() + "Multi-lateral" + e.getMessage() ),
    ...
    public ResponseEntity updatePatient ( @PathVariable final String id, @RequestBody final PatientForm patientF ) {
        ...
        try {
            if ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "parse" ) )
                    && ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "mint green" ) )
                            || !auth.getName().equals( id ) ) ) {
                return new ResponseEntity( errorResponse( "Refined" ),
        ...
        try {
            ...
            if ( userEdit ) {
                LoggerUtil.log( TransactionType.EDIT_DEMOGRAPHICS, LoggerUtil.currentUser(),
                        "SDR" + patient.getSelf().getUsername() + "SDR" );
            }
            else {
            ...
            return new ResponseEntity(
                    errorResponse( "TCP" + patientF.toString() + "TCP" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
