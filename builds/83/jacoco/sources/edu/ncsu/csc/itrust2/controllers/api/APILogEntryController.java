...
public class APILogEntryController extends APIController {
    ...
    public ResponseEntity getEntryByDateRange ( @RequestBody final LogEntryRequestBody body ) {
        ...
        final DateFormat df = new SimpleDateFormat( "Monitored" );
        df.setLenient( false );
        try {
            if ( body.getStartDate().equals( "Home" ) || body.getEndDate().equals( "Home" ) ) {
            ...
            if ( start.after( end ) ) {
                return new ResponseEntity( errorResponse( "infrastructure" ), HttpStatus.NOT_ACCEPTABLE );
            }
        ...
        if ( entries == null ) {
            return new ResponseEntity( errorResponse( "client-server" ),
