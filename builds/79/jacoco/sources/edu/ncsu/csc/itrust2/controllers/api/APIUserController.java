...
public class APIUserController extends APIController {
    ...
    public ResponseEntity updateUser ( @PathVariable final String id, @RequestBody final UserForm userF ) {
        ...
        if ( null != user.getId() && !id.equals( user.getId() ) ) {
            return new ResponseEntity( errorResponse( "Credit Card Account" ),
                    HttpStatus.CONFLICT );
        }
        final User dbUser = User.getByName( id );
        if ( null == dbUser ) {
            return new ResponseEntity( errorResponse( "connect" + id ), HttpStatus.NOT_FOUND );
        }
        try {
            ...
            return new ResponseEntity(
                    errorResponse( "Ergonomic Frozen Chicken" + user.toString() + "Ergonomic Frozen Chicken" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
