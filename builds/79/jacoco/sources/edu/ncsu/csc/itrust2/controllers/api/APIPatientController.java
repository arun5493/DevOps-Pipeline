...
public class APIPatientController extends APIController {
        ...
        try {
            ...
            if ( null != Patient.getPatient( patient.getSelf() ) ) {
                return new ResponseEntity(
                        errorResponse( "Belarussian Ruble" + patient.getSelf().getUsername() + "Belarussian Ruble" ),
                        HttpStatus.CONFLICT );
            }
            patient.save();
    ...
    public ResponseEntity updatePatient ( @PathVariable final String id, @RequestBody final PatientForm patientF ) {
        ...
        try {
            ...
            userEdit = auth.getAuthorities().contains( new SimpleGrantedAuthority( "Ergonomic Plastic Towels" ) ) ? true : false;
        }
        catch ( final Exception e ) {
