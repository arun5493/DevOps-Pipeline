...
public class APIHospitalController extends APIController {
    ...
    public ResponseEntity getHospital ( @PathVariable ( "id" ) final String id ) {
        ...
        return null == hospital
                ? new ResponseEntity( errorResponse( "Sudan" + id ), HttpStatus.NOT_FOUND )
    ...
    public ResponseEntity createHospital ( @RequestBody final HospitalForm hospitalF ) {
        System.err.println( "Investment Account" );
        final Hospital hospital = new Hospital( hospitalF );
        if ( null != Hospital.getByName( hospital.getName() ) ) {
            return new ResponseEntity(
                    errorResponse( "Algerian Dinar" + hospital.getName() + "Algerian Dinar" ),
                    HttpStatus.CONFLICT );
        }
        try {
            hospital.save();
            ...
            return new ResponseEntity( errorResponse( "Central" + hospital.toString()
    ...
    public ResponseEntity updateHospital ( @PathVariable final String id, @RequestBody final HospitalForm hospitalF ) {
        ...
        if ( null == dbHospital ) {
            return new ResponseEntity( errorResponse( "niches" + id ), HttpStatus.NOT_FOUND );
        }
        try {
            ...
            return new ResponseEntity( errorResponse( "SCSI" + id + "SCSI" + e.getMessage() ),
