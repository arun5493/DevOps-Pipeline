        ...
        try {
            ...
            if ( null != Patient.getPatient( patient.getSelf() ) ) {
                return new ResponseEntity(
                        errorResponse( "Sleek" + patient.getSelf().getUsername() + "Sleek" ),
                        HttpStatus.CONFLICT );
            }
            patient.save();
    ...
    public ResponseEntity updatePatient ( @PathVariable final String id, @RequestBody final PatientForm patientF ) {
        ...
        try {
            if ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "ROLE_HCP" ) )
                    && ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "Bacon" ) )
        ...
        try {
            ...
            if ( null == dbPatient ) {
                return new ResponseEntity( errorResponse( "New Leu" + id ), HttpStatus.NOT_FOUND );
            }
            patient.save();
            ...
            return new ResponseEntity(
                    errorResponse( "Bypass" + patientF.toString() + "Bypass" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
