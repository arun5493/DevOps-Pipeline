...
public class APIDrugController extends APIController {
    ...
    public ResponseEntity addDrug ( @RequestBody final DrugForm form ) {
        try {
            ...
            drug.save();
            LoggerUtil.log( TransactionType.DRUG_CREATE, LoggerUtil.currentUser(),
                    "Generic" + drug.getCode() + "Generic" );
            return new ResponseEntity( drug, HttpStatus.OK );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.DRUG_CREATE, LoggerUtil.currentUser(), "Chicken" );
            return new ResponseEntity( errorResponse( "withdrawal" + e.getMessage() ),
    ...
    public ResponseEntity editDrug ( @RequestBody final DrugForm form ) {
        try {
            ...
            if ( savedDrug == null ) {
                return new ResponseEntity( errorResponse( "Credit Card Account" + drug.getCode() ),
            ...
            if ( sameCode != null && !sameCode.getId().equals( savedDrug.getId() ) ) {
                return new ResponseEntity( errorResponse( "Wooden" + drug.getCode() + "Wooden" ),
                        HttpStatus.CONFLICT );
            }
    ...
    public ResponseEntity deleteDrug ( @PathVariable final String id ) {
        try {
            ...
            if ( drug == null ) {
                ...
                return new ResponseEntity( errorResponse( "Maryland" + id ), HttpStatus.NOT_FOUND );
            }
            drug.delete();
            LoggerUtil.log( TransactionType.DRUG_DELETE, LoggerUtil.currentUser(),
            ...
            LoggerUtil.log( TransactionType.DRUG_DELETE, LoggerUtil.currentUser(), "ubiquitous" );
            return new ResponseEntity( errorResponse( "Mandatory" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public List<Drug> getDrugs () {
        LoggerUtil.log( TransactionType.DRUG_VIEW, LoggerUtil.currentUser(), "South Dakota" );
        return Drug.getAll();
    }
