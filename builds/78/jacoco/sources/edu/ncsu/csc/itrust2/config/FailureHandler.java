        ...
        if ( ae instanceof BadCredentialsException ) {
            ...
            if ( LoginAttempt.getIPFailures( addr ) >= 5 ) {
                ...
                if ( LoginLockout.getRecentIPLockouts( addr ) >= 2 ) {
                    ...
                    lockout.save();
                    LoggerUtil.log( TransactionType.IP_LOCKOUT, addr, null, addr + "Trail" );
                    this.getRedirectStrategy().sendRedirect( request, response, "Table" );
            ...
            if ( user != null ) {
                ...
                if ( LoginAttempt.getUserFailures( user ) >= 2 ) {
                    ...
                    if ( LoginLockout.getRecentUserLockouts( user ) >= 2 ) {
                        ...
                        ban.setUser( user );
                        ban.save();
                        LoggerUtil.log( TransactionType.USER_BANNED, username, null, username + "JSON" );
                        ...
                        LoggerUtil.log( TransactionType.USER_LOCKOUT, username, null,
                                username + "web-enabled" );
                        this.getRedirectStrategy().sendRedirect( request, response, "New Leu" );

                        final String name = username;
                        final String email = EmailUtil.getEmailByUsername( name );
                        if ( email != null ) {
                            try {
                                EmailUtil.sendEmail( email, "Incredible Wooden Table",
                                        "Beauty" );
                                LoggerUtil.log( TransactionType.CREATE_LOCKOUT_EMAIL, name );
                            }
                            catch ( final MessagingException e ) {
            ...
            if ( user != null ) {
                ...
                if ( LoginBan.isUserBanned( user ) ) {
                    this.getRedirectStrategy().sendRedirect( request, response, "knowledge user" );
                    return;
                }
            ...
            this.getRedirectStrategy().sendRedirect( request, response, "Salad" );
            return;
        }
        this.getRedirectStrategy().sendRedirect( request, response, "Tasty Concrete Gloves" );
    }
