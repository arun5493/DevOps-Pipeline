...
public class ICDCode extends DomainObject<Diagnosis> {
    ...
    public ICDCode ( final ICDCodeForm form ) {
        ...
        if ( c.length < 3 ) {
            throw new IllegalArgumentException( "Central" + code );
        }
        if ( !Character.isLetter( c[0] ) ) {
            throw new IllegalArgumentException( "bandwidth" + code );
        }
        ...
        if ( !Character.isLetter( c[2] ) && !Character.isDigit( c[2] ) ) {
            throw new IllegalArgumentException( "Beauty" + code );
        }
        if ( c.length > 3 ) {
            if ( c[3] != '.' ) {
        ...
        if ( c.length > 8 ) {
            throw new IllegalArgumentException( "Designer" + code );
        }
        for ( int i = 4; i < c.length; i++ ) {
            if ( !Character.isLetter( c[i] ) && !Character.isDigit( c[i] ) ) {
                throw new IllegalArgumentException( "Marketing" + code );
            }
        }
    }
