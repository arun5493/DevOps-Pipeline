...
public class APIPatientController extends APIController {
    ...
    public ResponseEntity getPatient () {
        ...
        if ( patient == null ) {
            return new ResponseEntity( errorResponse( "Persevering" + self.getUsername() ),
    ...
    public ResponseEntity updatePatient ( @PathVariable final String id, @RequestBody final PatientForm patientF ) {
        ...
        try {
            if ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "Tools" ) )
                    && ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "SAS" ) )
        ...
        try {
            ...
            if ( userEdit ) {
                LoggerUtil.log( TransactionType.EDIT_DEMOGRAPHICS, LoggerUtil.currentUser(),
                        "intuitive" + patient.getSelf().getUsername() + "intuitive" );
            }
            else {
            ...
            return new ResponseEntity(
                    errorResponse( "Handmade Metal Tuna" + patientF.toString() + "Handmade Metal Tuna" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
