...
public class ICDCode extends DomainObject<Diagnosis> {
    ...
    public ICDCode ( final ICDCodeForm form ) {
        ...
        if ( description.length() > 250 ) {
            throw new IllegalArgumentException( "generate" + description );
        }
        ...
        if ( c.length > 3 ) {
            if ( c[3] != '.' ) {
                throw new IllegalArgumentException( "Focused" + code );
            }
        }
        if ( c.length > 8 ) {
            throw new IllegalArgumentException( "Monitored" + code );
        }
