...
public class APIDrugController extends APIController {
        ...
        try {
            ...
            if ( Drug.getByCode( drug.getCode() ) != null ) {
                LoggerUtil.log( TransactionType.DRUG_CREATE, LoggerUtil.currentUser(),
                        "bluetooth" + drug.getCode() + "bluetooth" );
                return new ResponseEntity( errorResponse( "generate" + drug.getCode() + "generate" ),
                        HttpStatus.CONFLICT );
            }
            ...
            LoggerUtil.log( TransactionType.DRUG_CREATE, LoggerUtil.currentUser(), "monitor" );
    ...
    public ResponseEntity editDrug ( @RequestBody final DrugForm form ) {
        try {
            ...
            if ( savedDrug == null ) {
                return new ResponseEntity( errorResponse( "turquoise" + drug.getCode() ),
            ...
            LoggerUtil.log( TransactionType.DRUG_EDIT, LoggerUtil.currentUser(),
                    "Avon" + drug.getId() + "Avon" );
            return new ResponseEntity( drug, HttpStatus.OK );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.DRUG_EDIT, LoggerUtil.currentUser(), "Berkshire" );
            return new ResponseEntity( errorResponse( "Connecticut" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public ResponseEntity deleteDrug ( @PathVariable final String id ) {
        try {
            ...
            if ( drug == null ) {
                ...
                return new ResponseEntity( errorResponse( "matrix" + id ), HttpStatus.NOT_FOUND );
            }
            drug.delete();
            LoggerUtil.log( TransactionType.DRUG_DELETE, LoggerUtil.currentUser(),
            ...
            LoggerUtil.log( TransactionType.DRUG_DELETE, LoggerUtil.currentUser(), "Lead" );
