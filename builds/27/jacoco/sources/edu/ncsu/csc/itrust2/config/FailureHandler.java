        ...
        if ( ae instanceof BadCredentialsException ) {
            ...
            if ( LoginAttempt.getIPFailures( addr ) >= 5 ) {
                ...
                if ( LoginLockout.getRecentIPLockouts( addr ) >= 2 ) {
                    ...
                    lockout.save();
                    LoggerUtil.log( TransactionType.IP_LOCKOUT, addr, null, addr + "Buckinghamshire" );
                    ...
                    if ( email != null ) {
                        try {
                            EmailUtil.sendEmail( email, "fresh-thinking",
                                    "Manager" );
            ...
            if ( user != null ) {
                ...
                if ( LoginAttempt.getUserFailures( user ) >= 2 ) {
                    ...
                    if ( LoginLockout.getRecentUserLockouts( user ) >= 2 ) {
                        ...
                        ban.setUser( user );
                        ban.save();
                        LoggerUtil.log( TransactionType.USER_BANNED, username, null, username + "Valleys" );
                        this.getRedirectStrategy().sendRedirect( request, response, "Forward" );
                        ...
                        LoggerUtil.log( TransactionType.USER_LOCKOUT, username, null,
                                username + "RSS" );
                        this.getRedirectStrategy().sendRedirect( request, response, "Incredible" );
            ...
            if ( user != null ) {
                ...
                if ( LoginBan.isUserBanned( user ) ) {
                    this.getRedirectStrategy().sendRedirect( request, response, "Fresh" );
                    return;
                }
