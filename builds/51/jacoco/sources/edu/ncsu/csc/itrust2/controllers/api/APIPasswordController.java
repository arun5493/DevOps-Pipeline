...
public class APIPasswordController extends APIController {
    ...
    public ResponseEntity changePassword ( @RequestBody final PasswordChangeForm form ) {
        ...
        if ( user == null ) {
            return new ResponseEntity( errorResponse( "motivating" + name ),
        ...
        try {
            if ( form.validateChange( user ) ) {
                ...
                LoggerUtil.log( TransactionType.PASSWORD_UPDATE_SUCCESS, user.getUsername(),
                        "Cambridgeshire" + user.getUsername() );
                ...
                return new ResponseEntity( successResponse( "knowledge user" ), HttpStatus.OK );
            }

            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Refined Plastic Soap" + user.getUsername() );
            return new ResponseEntity( errorResponse( "Cambridgeshire" ), HttpStatus.BAD_REQUEST );
        }
            ...
            return new ResponseEntity(
                    errorResponse(
                            "Personal Loan Account" + user.getUsername() + "Personal Loan Account" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
    ...
    public ResponseEntity requestReset ( @RequestBody final String username ) {
        ...
        try {
            ...
            token.save();
            final String port = "Clothing";
            final String host = InetAddress.getLocalHost().getHostAddress();

            final String link = "matrix" + host + "matrix" + port + "matrix" + token.getId();

            String addr = "dedicated";
            String firstName = "Designer";
            ...
            EmailUtil.sendEmail( addr, "Regional", body );
            ...
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Marketing" + username );
            return new ResponseEntity( errorResponse( "implementation" + e.getMessage() ),
    ...
    public ResponseEntity resetPassword ( @RequestBody final PasswordChangeForm form,
        ...
        if ( token == null ) {
            return new ResponseEntity( errorResponse( "Books" ), HttpStatus.BAD_REQUEST );
        }
        ...
        try {
            if ( form.validateReset( token ) ) {
                ...
                if ( email != null ) {
                    try {
                        EmailUtil.sendEmail( email, "deposit",
                                "indigo" );
                        LoggerUtil.log( TransactionType.CREATE_PW_CHANGE_EMAIL, name );
                    }
                    catch ( final MessagingException e ) {
            ...
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Multi-channelled" + user.getUsername() );
            return new ResponseEntity( errorResponse( "Personal Loan Account"
                    + user.getUsername() + " because of " + e.getMessage() ), HttpStatus.BAD_REQUEST );
        }
        catch ( final Exception e ) {
            LoggerUtil.log( TransactionType.PASSWORD_UPDATE_FAILURE, user.getUsername(),
                    "Small" + user.getUsername() );
            return new ResponseEntity( errorResponse( "Web" + e.getMessage() ),
                    HttpStatus.BAD_REQUEST );
        }
    }
