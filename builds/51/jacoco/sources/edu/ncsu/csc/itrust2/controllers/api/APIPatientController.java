...
public class APIPatientController extends APIController {
        ...
        try {
            ...
            if ( null != Patient.getPatient( patient.getSelf() ) ) {
                return new ResponseEntity(
                        errorResponse( "Object-based" + patient.getSelf().getUsername() + "Object-based" ),
                        HttpStatus.CONFLICT );
            }
            patient.save();
            ...
            return new ResponseEntity(
                    errorResponse( "navigating" + patientF.toString() + "navigating" + e.getMessage() ),
    ...
    public ResponseEntity updatePatient ( @PathVariable final String id, @RequestBody final PatientForm patientF ) {
        ...
        try {
            if ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "green" ) )
                    && ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "Direct" ) )
                            || !auth.getName().equals( id ) ) ) {
                return new ResponseEntity( errorResponse( "magenta" ),
        ...
        try {
            ...
            if ( null == dbPatient ) {
                return new ResponseEntity( errorResponse( "Analyst" + id ), HttpStatus.NOT_FOUND );
            }
            patient.save();
